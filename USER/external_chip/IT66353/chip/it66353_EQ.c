...
{
    return it6635_get_port_info1( g_pstDev->vars.Rx_active_port, PI_PLL_HS1G );
}
...
void _rx_ced_err_recheck( void )
{
    ...
    for( i=0 ; i<3 ; i++ )
    {
        if ( (g_pstDev->vars.RxCEDErrValid&(1<<i)) && (g_pstDev->vars.RxCEDErr[i] > 0x100) )
        {
...
void _rx_update_ced_err_from_hw(void)
{
    ...
    if ( _rx_is_clock_stable() )
    {
        ...
        else
        {
            ...
            {
                h2rxwr(0xe9, 0x00);
                g_pstDev->vars.RxCEDErr[0] = h2rxrd(0xEB);

                if ( g_pstDev->vars.RxCEDErr[0] & 0x80 ) // ced valid
                {
                    g_pstDev->vars.RxCEDErr[0] = g_pstDev->vars.RxCEDErr[0] & 0x7F;
                    g_pstDev->vars.RxCEDErr[0] = (g_pstDev->vars.RxCEDErr[0]<<8) + h2rxrd(0xEA);
                    g_pstDev->vars.RxCEDErrValid=1;
                }
                else
                {
                    g_pstDev->vars.RxCEDErrValid=0;
            ...
            {
                h2rxwr(0xe9, 0x20);
                g_pstDev->vars.RxCEDErr[1] = h2rxrd(0xEB);

                if ( g_pstDev->vars.RxCEDErr[1] & 0x80 )
                {
                    g_pstDev->vars.RxCEDErr[1] = g_pstDev->vars.RxCEDErr[1] & 0x7F;
                    g_pstDev->vars.RxCEDErr[1] = (g_pstDev->vars.RxCEDErr[1]<<8) + h2rxrd(0xEA);
                    g_pstDev->vars.RxCEDErrValid |= 0x02;
                }
                else
                {
                    g_pstDev->vars.RxCEDErrValid &= ~0x02;
            ...
            {
                h2rxwr(0xe9, 0x40);
                g_pstDev->vars.RxCEDErr[2] = h2rxrd(0xEB);

                if ( g_pstDev->vars.RxCEDErr[2] & 0x80 )
                {
                    g_pstDev->vars.RxCEDErr[2] = g_pstDev->vars.RxCEDErr[2] & 0x7F;
                    g_pstDev->vars.RxCEDErr[2] = (g_pstDev->vars.RxCEDErr[2]<<8) + h2rxrd(0xEA);
                    g_pstDev->vars.RxCEDErrValid |= 0x04;
                }
                else
                {
                    g_pstDev->vars.RxCEDErrValid &= ~0x04;
...
void _rx_record_ced_err(void)
{
    ...
    if ( g_pstDev->EQ.ced_acc_count == 0 )
    {
        for (i=0 ; i<3 ; i++)
        {
            g_pstDev->EQ.ced_err_avg[i] = g_pstDev->vars.RxCEDErr[i];
            //g_pstDev->EQ.ced_err_avg_prev[i] = 0x8888;
        }
    }
    else
    {
        for (i=0 ; i<3 ; i++)
        {
            g_pstDev->EQ.ced_err_avg[i] = g_pstDev->EQ.ced_err_avg[i] + g_pstDev->vars.RxCEDErr[i];
            g_pstDev->EQ.ced_err_avg[i]>>=1;
        }
    }
    g_pstDev->EQ.ced_acc_count++;
}
...
u8 _rx_monitor_ced_err(void)
{
    ...
    if ( g_pstDev->vars.RxCEDErrValid )
    {
        if ( (g_pstDev->vars.RxCEDErr[0] || g_pstDev->vars.RxCEDErr[1] || g_pstDev->vars.RxCEDErr[2]) )
        {
            PR_INFO(("CED=0x%04X, 0x%04X, 0x%04X\r\n", g_pstDev->vars.RxCEDErr[0], g_pstDev->vars.RxCEDErr[1], g_pstDev->vars.RxCEDErr[2]));
            ...
            {
                err_acc = 0;
                if ( (g_pstDev->vars.RxCEDErr[0]>0x08 || g_pstDev->vars.RxCEDErr[1]>0x08 || g_pstDev->vars.RxCEDErr[2]>0x08) )
                {
...
{
    #if 1
    return g_pstDev->vars.clock_ratio;
    #else
    if( h2swrd(0x61+ g_pstDev->vars.Rx_active_port *3)&0x40 )
    {
        return 1;
    }
    return 0;
...
{
	chgrxbank(3);
	g_pstDev->EQ.stored_RS_20[0] = h2rxrd(0x27);
	g_pstDev->EQ.stored_RS_20[1] = h2rxrd(0x28);
	g_pstDev->EQ.stored_RS_20[2] = h2rxrd(0x29);
	chgrxbank(0);
}
...
void _eq_load_h20(void)
{
	...
	_rx_set_rs_3ch( g_pstDev->EQ.stored_RS_20 );
	_aeq_set_DFE2( g_pstDev->EQ.stored_RS_20[0], g_pstDev->EQ.stored_RS_20[1], g_pstDev->EQ.stored_RS_20[2] );
	_rx_DFE_enable(1);
}
...
{
	chgrxbank(3);
	g_pstDev->EQ.stored_RS_14[0] = h2rxrd(0x27);
	g_pstDev->EQ.stored_RS_14[1] = h2rxrd(0x28);
	g_pstDev->EQ.stored_RS_14[2] = h2rxrd(0x29);
	chgrxbank(0);
}
...
void _eq_load_h14(void)
{
	...
	_rx_set_rs_3ch( g_pstDev->EQ.stored_RS_14 );
}
...
void _eq_load_previous(void)
{
	if ( _rx_is_hdmi20() )
	{
		if (g_pstDev->EQ.current_eq_mode == 0) {
			g_pstDev->EQ.current_eq_mode = 1;
			_eq_load_h20();
		}
	}
	else
	{
		if (g_pstDev->EQ.current_eq_mode == 1) {
			g_pstDev->EQ.current_eq_mode = 0;
			_eq_load_h14();
		}
	}
}
...
{
    _rx_set_rs_3ch( g_pstDev->opts.active_rx_opt->DefaultEQ );
...
{
    g_pstDev->EQ.txoe_ready20 = 0;
    g_pstDev->EQ.txoe_ready14 = 0;
}
...
void _eq_set_txoe_ready(u8 ready)
{
	if ( _rx_is_hdmi20() )
	{
		g_pstDev->EQ.txoe_ready20 = ready;
	}
	else
	{
		g_pstDev->EQ.txoe_ready14 = ready;
	}

    PR_INFO(("set ready 14=%d 20=%d\r\n", g_pstDev->EQ.txoe_ready14, g_pstDev->EQ.txoe_ready20));
}
...
u8 _eq_get_txoe_ready(void)
{
	...
	if ( _rx_is_hdmi20() )
	{
		return g_pstDev->EQ.txoe_ready20;
	}
	else
	{
		return g_pstDev->EQ.txoe_ready14;
	}
}

void _eq_reset_state(void)
{
    g_pstDev->EQ.EQ_flag_20 = SysAEQ_RUN;
    g_pstDev->EQ.EQ_flag_14 = SysAEQ_RUN;
}
...
void _eq_set_state(u8 state)
{
	if ( _rx_is_hdmi20() )
	{
		g_pstDev->EQ.EQ_flag_20 = state;
	}
	else
	{
		g_pstDev->EQ.EQ_flag_14 = state;
	}

    PR_INFO(("set eq flag 14=%d 20=%d\r\n", g_pstDev->EQ.EQ_flag_14, g_pstDev->EQ.EQ_flag_20));
}
...
u8 _eq_get_state(void)
{
	...
	if ( _rx_is_hdmi20() )
	{
		return g_pstDev->EQ.EQ_flag_20;
	}
	else
	{
		return g_pstDev->EQ.EQ_flag_14;
	}
}
...
void _aeq_get_DFE2(u8 ch, u8 rs_idx)
{
    ...
    for( ch=0 ; ch<3 ; ch++ )
    {
        ...
        g_pstDev->EQ.DFE[i][ch][0] = dfe_a;
        g_pstDev->EQ.DFE[i][ch][1] = dfe_b;
        g_pstDev->EQ.DFE[i][ch][2] = dfe_c;
...
void _aeq_set_DFE2(u8 EQ0, u8 EQ1, u8 EQ2)
{
    ...
    _aeq_get_DFE2(0, i);
    dfe[0] = g_pstDev->EQ.DFE[i][Channel_B][0];
    dfe[1] = g_pstDev->EQ.DFE[i][Channel_B][1];
    dfe[2] = g_pstDev->EQ.DFE[i][Channel_B][2];

	i = _eq_find_rs_index(EQ1);
    _aeq_get_DFE2(1, i);
    dfe[3] = g_pstDev->EQ.DFE[i][Channel_G][0];
    dfe[4] = g_pstDev->EQ.DFE[i][Channel_G][1];
    dfe[5] = g_pstDev->EQ.DFE[i][Channel_G][2];

	i = _eq_find_rs_index(EQ2);
    _aeq_get_DFE2(2, i);
    dfe[6] = g_pstDev->EQ.DFE[i][Channel_R][0];
    dfe[7] = g_pstDev->EQ.DFE[i][Channel_R][1];
    dfe[8] = g_pstDev->EQ.DFE[i][Channel_R][2];
...
{
    chgrxbank(3);
    h2rxbrd( 0xD5, 3, g_pstDev->EQ.CalcRS );
    chgrxbank(0);
    PR_AEQ(("AEQ RS=%02X %02X %02X\r\n", g_pstDev->EQ.CalcRS[0], g_pstDev->EQ.CalcRS[1], g_pstDev->EQ.CalcRS[2] ));
}
...
void _aeq_check_amp_and_rs_status(void)
{
    ...
    g_pstDev->EQ.DFE_Valid = 1;
    g_pstDev->EQ.RS_Valid = 1;
	...
	for(i=0;i<3;i++)
	{
        ...
        if ( g_pstDev->EQ.RS_ValidMap[i] == 0x0000 )
        {
            g_pstDev->EQ.RS_Valid = 0;
        }
	}
    chgrxbank(0);
}
...
u8 _aeq_check_sareq_result(void)
{
    ...
    if ( tmp & (BIT5|BIT4) ) // EQ fail | EQ Done
    {
        ...
        if ( tmp & BIT4 ) {
            g_pstDev->EQ.AutoEQ_Result = EQRES_SAREQ_DONE;
        }
        else{
            g_pstDev->EQ.AutoEQ_Result = EQRES_SAREQ_FAIL;
        }
    ...
    */
    ...
    {
        g_pstDev->EQ.AutoEQ_WaitTime++;
        delay1ms(20);
        if ( g_pstDev->EQ.AutoEQ_WaitTime > 30 )
        {
            g_pstDev->EQ.AutoEQ_Result = EQRES_SAREQ_TIMEOUT;
            return 1;
        }
    }
...
u8 _aeq_check_h14eq_result(void)
{
    ...
    else
    {
        delay1ms(25);
        g_pstDev->EQ.AutoEQ_WaitTime++;
        if ( g_pstDev->EQ.AutoEQ_WaitTime > 100 )
        {
            g_pstDev->EQ.AutoEQ_Result = EQRES_SAREQ_TIMEOUT;
...
u8 _meq_14(void)
{
	...
	for ( rs_idx = 0; rs_idx<14 ; rs_idx++ )
	{
		...
		for (ch = 0; ch < 3; ch++)
		{
            ...
            if ( g_pstDev->vars.RxCEDErrValid & (1<<ch) )
            {
                if ( g_pstDev->vars.RxCEDErr[ch] < 0x01 )
                {
                    ch_done[ch]=1;
                    ch_all_done++;
                ...
                {
                    ch_done[ch]=0;
                    PR_AEQ(("RS NG ch%d = %02X, err=%04X\r\n", ch, RS_Value[ ch_rs_idx[ch] ], g_pstDev->vars.RxCEDErr[ch] ));
                }
            }
            else
            {
...
bool it6635_auto_eq_adjust(void)
{
	...
	if ( _aeq_ready_to_start() )
	{
        ...
        if ( g_pstDev->vars.count_auto_eq_fail > 13 )
        {
            g_pstDev->vars.count_auto_eq_fail = 0;
            _rx_caof_init(g_pstDev->vars.Rx_active_port);
        }

        if ( _rx_is_hdmi20() )
		{
			g_pstDev->EQ.current_eq_mode = EQ_MODE_H20;
            g_pstDev->EQ.RS_Valid = 0;
            g_pstDev->EQ.DFE_Valid = 0;
			_aeq_trigger_SAREQ();
            ...
            if ( g_pstDev->EQ.DFE_Valid && g_pstDev->EQ.RS_Valid )
            {
                _aeq_get_calc_rs();
                _rx_set_rs_3ch( g_pstDev->EQ.CalcRS );
                _aeq_set_DFE2( g_pstDev->EQ.CalcRS[0], g_pstDev->EQ.CalcRS[1], g_pstDev->EQ.CalcRS[2] );
                _rx_DFE_enable(1);
		...
		{
			g_pstDev->EQ.current_eq_mode = EQ_MODE_H14;
        ...
        {
            g_pstDev->vars.count_auto_eq_fail++;
