...
void DumpiT6601Reg(void)
{
    ...
    for ( i = 0 ; i < 0x4F ; i+=16 )
    {
        dbg_printf("[%3bX]  ",(BYTE)i) ;
        for ( j = 0 ; j < 16 ; j++ )
        {
            ucData = CEC_ReadI2C_Byte((BYTE)((i+j)&0xFF)) ;
            dbg_printf(" %2bX", ucData) ;
            if ( (j == 3)||(j==7)||(j==11) )
            {
                dbg_printf(" -") ;
            }
...
void cecclrint( void )
{
 ...
 USHORT  ucTickCount;//shenzy CEC暂时不用，为了编译通过，设一个全局变量。
...
#ifdef DEBUG_iT6601
    dbg_printf("CECCmdSet --> Tx Header:%bX, opcode:%bX , operand1:%bX , operand2:%bX ,Size:%bX-----\n ",
           TxCmdBuf.HEADER,
           TxCmdBuf.OPCODE,
           TxCmdBuf.OPERAND1,
           TxCmdBuf.OPERAND2,
...
//=== ARC ====
        case eInitiateARC :
            dbg_printf("Receive !!!Initiate ARC !!!\n");
//            if(CheckUpDownPhyAdr()==TRUE)
        ...
        case eReportARCInitiated :
            dbg_printf("Receive !!!Report ARC Initiated !!!\n");
            break;
        case eReportARCTerminated :
            dbg_printf("Receive !!!Report ARC Terminated !!!\n");
...
//            DumpiT6601Reg();
        ...
        case eRequestARCInitiation :
            dbg_printf("Receive !!!Request ARC Initiation !!!\n");
//            if(CheckUpDownPhyAdr()==TRUE)
            {
        ...
        case eRequestARCTermination :    //Request ARC Termination
            dbg_printf("Receive !!!Request ARC Termination !!!\n");
            if ( CheckUpDownPhyAdr()==TRUE )
            {
        ...
        case eTerminateARC :    //Termiate ARC
            dbg_printf("Receive !!!Termiate ARC !!!\n");
...
void Switch_SystemAudioControl(SystemAudioControl_StateType State)
{
    ...
    switch ( SACState )
    {
        ...
        case sSetSystemAudioModeToTV:
            ...
            //CreatTimerTask(eCECTimeOut,200,SysTimerTask_OnceRun);//shenzy CEC暂时不用
            break;
        case sWaitingFeatureAbort:
           // CreatTimerTask(eCECTimeOut,1000,SysTimerTask_OnceRun);//shenzy CEC暂时不用
            break;
...
void SystemAudioControl(void)       //for Audio control system CTS 11.2.15
{
    switch ( SACState )
    {
        ...
        case sSetSystemAudioModeToTV:{
                if ( OldTxCECcmd==eSetSystemAudioMode )
                {
                    switch ( CECTxState )
                    {
                        ...
                        case sCECReceiverNack:
                            IT6601_PRINT(("!!! Nack !!!\n"));
                           // if ( TimeOutCheck(eCECTimeOut)==TRUE )//CEC暂时不用
                                Switch_SystemAudioControl(sSetSystemAudioModeToAll);
                            break;
                        case sCECOk:
                        case sCECNone:
        ...
        case sWaitingFeatureAbort:{
                ...
                {
                   //if ( TimeOutCheck(eCECTimeOut)==TRUE )//shenzy 编译通过cec暂时不用。
                        Switch_SystemAudioControl(sSetSystemAudioModeToAll);
                }
            }
            break;
...
// for iT6633+iT6623                        dbg_printf("CEC volume = %bX \n",uCECAudioStatus.Map.AudioVolumeStatus);
            }
            else
            {
                if ( (uCECAudioStatus.Map.AudioVolumeStatus) == 0x00 )
