...
BOOL HDMITX_EnableHDCP(BYTE dev ,BYTE bEnable)
{
#ifdef SUPPORT_HDCP
    if(bEnable)
    {
        ...
        {
            //dbg_printf("ER_FAIL == hdmitx_hdcp_Authenticate\n");
...
BOOL getHDMITX_AuthenticationDone(BYTE dev)
{
    //IT66121_HDCP_DEBUG_PRINTF((" getHDMITX_AuthenticationDone() = %s\n",hdmiTxDev[0].bAuthenticated?"TRUE":"FALSE" ));
    return hdmiTxDev[dev].bAuthenticated;
}
...
{
    // IT66121_HDCP_DEBUG_PRINTF(("hdmitx_hdcp_Auth_Fire():\n"));
...
SYS_STATUS hdmitx_hdcp_GetBCaps(PBYTE pBCaps ,PUSHORT pBStatus)
{
    ...
    for(TimeOut = 200 ; TimeOut > 0 ; TimeOut --)
    {
        ...
        {
            //IT66121_HDCP_DEBUG_PRINTF(("hdmitx_hdcp_GetBCaps(): DDC Done.\n"));
            break ;
        }
        if(ucdata & B_TX_DDC_ERROR)
        {
//            IT66121_HDCP_DEBUG_PRINTF(("hdmitx_hdcp_GetBCaps(): DDC fail by reg16=%02X.\n",ucdata));
            return ER_FAIL ;
        }
    }
    if(TimeOut == 0)
    ...
    *pBStatus |= (int)(HDMITX_ReadI2C_Byte(0x17)&0xFF)<<8;
    IT66121_HDCP_DEBUG_PRINTF(("hdmitx_hdcp_GetBCaps(): ucdata = %02X\n",(int)HDMITX_ReadI2C_Byte(0x16)));
#endif
    return ER_SUCCESS ;
}
...
SYS_STATUS hdmitx_hdcp_GetBKSV(BYTE *pBKSV)
{
    ...
    for(TimeOut = 200 ; TimeOut > 0 ; TimeOut --)
    {
        ...
        if(ucdata & B_TX_DDC_ERROR)
        {
            IT66121_HDCP_DEBUG_PRINTF(("hdmitx_hdcp_GetBCaps(): DDC No ack or arbilose,%x,maybe cable did not connected. Fail.\n",ucdata));
            return ER_FAIL ;
        }
    }
    if(TimeOut == 0)
...
SYS_STATUS hdmitx_hdcp_Authenticate(BYTE dev)
{
    BYTE ucdata ;
    BYTE BCaps ;
    USHORT BStatus ;
    ...
    IT66121_HDCP_DEBUG_PRINTF(("hdmitx_hdcp_Authenticate():\n"));
    hdmitx_hdcp_Reset();
    ...
    for( TimeOut = 0 ; TimeOut < 80 ; TimeOut++ )
    {
        ...
        {
            IT66121_HDCP_DEBUG_PRINTF(("hdmitx_hdcp_GetBCaps fail.\n"));
            return ER_FAIL ;
        }
        // IT66121_HDCP_DEBUG_PRINTF(("(%d)Reg16 = %02X\n",idx++,(int)HDMITX_ReadI2C_Byte(0x16)));
    ...
    {
        IT66121_HDCP_DEBUG_PRINTF(("Down Stream Count %d is over maximum supported number 6,fail.\n",(int)(BStatus & M_TX_DOWNSTREAM_COUNT)));
        return ER_FAIL ;
    }
    */
	IT66121_HDCP_DEBUG_PRINTF(("BCAPS = %02X BSTATUS = %04X\n", (int)BCaps, BStatus));
    hdmitx_hdcp_GetBKSV(BKSV);
    IT66121_HDCP_DEBUG_PRINTF(("BKSV %02X %02X %02X %02X %02X\n",(int)BKSV[0],(int)BKSV[1],(int)BKSV[2],(int)BKSV[3],(int)BKSV[4]));
    ...
    if( ucdata != 20 )
    {
        IT66121_HDCP_DEBUG_PRINTF(("countbit error\n"));
        return ER_FAIL ;
    ...
    if((BCaps & B_TX_CAP_HDMI_REPEATER) == 0)
    {
        ...
        {
            it66121_delay_ms(5); // delay 1ms
            ucdata = HDMITX_ReadI2C_Byte(REG_TX_AUTH_STAT);
            // IT66121_HDCP_DEBUG_PRINTF(("reg46 = %02x reg16 = %02x\n",(int)ucdata,(int)HDMITX_ReadI2C_Byte(0x16)));

            if(ucdata & B_TX_AUTH_DONE)
            {
                hdmiTxDev[dev].bAuthenticated = TRUE ;
                break ;
            }
            ...
            if(ucdata & B_TX_INT_AUTH_FAIL)
            {
                ...
                IT66121_HDCP_DEBUG_PRINTF(("hdmitx_hdcp_Authenticate()-receiver: Authenticate fail\n"));
                hdmiTxDev[dev].bAuthenticated = FALSE ;
                return ER_FAIL ;
            }
        }
        if(TimeOut == 0)
        {
             IT66121_HDCP_DEBUG_PRINTF(("hdmitx_hdcp_Authenticate()-receiver: Time out. return fail\n"));
             hdmiTxDev[dev].bAuthenticated = FALSE ;
             return ER_FAIL ;
        }
        return ER_SUCCESS ;
...
SYS_STATUS hdmitx_hdcp_VerifyIntegration(BYTE dev)
{
    ...
    if(hdmiTxDev[dev].bAuthenticated == TRUE)
    {
        return ER_SUCCESS ;
    }
    return ER_FAIL ;
...
{
    IT66121_HDCP_DEBUG_PRINTF(("hdmitx_hdcp_CancelRepeaterAuthenticate"));
    HDMITX_WriteI2C_Byte(REG_TX_DDC_MASTER_CTRL,B_TX_MASTERDDC|B_TX_MASTERHOST);
    hdmitx_AbortDDC();
...
//             //IT66121_HDCP_DEBUG_PRINTF(("w[%2d] = %08lX\n",t,w[t%SHA_BUFF_COUNT]));
//         }
//
//         if((t>=0)&&(t<20)) {
//             tmp = rol(h[0],5) + ((h[1] & h[2]) | (h[3] & ~h[1])) + h[4] + w[t%SHA_BUFF_COUNT] + 0x5a827999;
//             //IT66121_HDCP_DEBUG_PRINTF(("%08lX %08lX %08lX %08lX %08lX\n",h[0],h[1],h[2],h[3],h[4]));
//
//             h[4] = h[3];
//             h[3] = h[2];
//             h[2] = rol(h[1],30);
...
//         c <<= (3-(i%4))*8 ;
//         w[t] |= c ;
// //        IT66121_HDCP_DEBUG_PRINTF(("pBuff[%d] = %02x, c = %08lX, w[%d] = %08lX\n",i,pBuff[i],c,t,w[t]));
//     }
//
//     t = i/4 ;
//     if( i%4 == 0 )
...
/*
    for(i = 0 ; i < 64 ; i++)
    {
        if(i % 16 == 0)
        {
            IT66121_HDCP_DEBUG_PRINTF(("SHA[]: "));
        }
        IT66121_HDCP_DEBUG_PRINTF((" %02X",SHABuff[i]));
        if((i%16)==15)
        {
            IT66121_HDCP_DEBUG_PRINTF(("\n"));
        }
    }
    */
    ...
    for(i = 0 ; i < 20 ; i++)
    {
        if(V[i] != Vr[i])
        {
            IT66121_HDCP_DEBUG_PRINTF(("V[] ="));
            for(i = 0 ; i < 20 ; i++)
            {
                IT66121_HDCP_DEBUG_PRINTF((" %02X",(int)V[i]));
            }
            IT66121_HDCP_DEBUG_PRINTF(("\nVr[] ="));
            for(i = 0 ; i < 20 ; i++)
            {
                IT66121_HDCP_DEBUG_PRINTF((" %02X",(int)Vr[i]));
            }
            return ER_FAIL ;
        }
    }
...
SYS_STATUS hdmitx_hdcp_GetKSVList(BYTE *pKSVList,BYTE cDownStream)
{
    ...
    for(TimeOut = 200 ; TimeOut > 0 ; TimeOut --)
    {
        ...
        if(ucdata & B_TX_DDC_DONE)
        {
            IT66121_HDCP_DEBUG_PRINTF(("hdmitx_hdcp_GetKSVList(): DDC Done.\n"));
            break ;
        }
        if(ucdata & B_TX_DDC_ERROR)
        {
            IT66121_HDCP_DEBUG_PRINTF(("hdmitx_hdcp_GetKSVList(): DDC Fail by REG_TX_DDC_STATUS = %x.\n",ucdata));
            return ER_FAIL ;
        }
        it66121_delay_ms(5);
    }
    ...
    IT66121_HDCP_DEBUG_PRINTF(("hdmitx_hdcp_GetKSVList(): KSV"));
    for(TimeOut = 0 ; TimeOut < cDownStream * 5 ; TimeOut++)
    {
        pKSVList[TimeOut] = HDMITX_ReadI2C_Byte(REG_TX_DDC_READFIFO);
        IT66121_HDCP_DEBUG_PRINTF((" %02X",(int)pKSVList[TimeOut]));
    }
    IT66121_HDCP_DEBUG_PRINTF(("\n"));
    return ER_SUCCESS ;
}
...
SYS_STATUS hdmitx_hdcp_GetVr(BYTE *pVr)
{
    ...
    for(TimeOut = 200 ; TimeOut > 0 ; TimeOut --)
    {
        ...
        if(ucdata & B_TX_DDC_DONE)
        {
            IT66121_HDCP_DEBUG_PRINTF(("hdmitx_hdcp_GetVr(): DDC Done.\n"));
            break ;
        }
        if(ucdata & B_TX_DDC_ERROR)
        {
            IT66121_HDCP_DEBUG_PRINTF(("hdmitx_hdcp_GetVr(): DDC fail by REG_TX_DDC_STATUS = %x.\n",(int)ucdata));
            return ER_FAIL ;
        }
        it66121_delay_ms(5);
    }
    if(TimeOut == 0)
    {
        IT66121_HDCP_DEBUG_PRINTF(("hdmitx_hdcp_GetVr(): DDC fail by timeout.\n"));
        return ER_FAIL ;
    }
    Switch_HDMITX_Bank(0);
...
SYS_STATUS hdmitx_hdcp_GetM0(BYTE *pM0)
{
    ...
    IT66121_HDCP_DEBUG_PRINTF(("M[] ="));
    for(i = 0 ; i < 8 ; i++)
    {
        IT66121_HDCP_DEBUG_PRINTF(("0x%02x,",(int)pM0[i]));
    }
    IT66121_HDCP_DEBUG_PRINTF(("\n"));
    return ER_SUCCESS ;
}
...
SYS_STATUS hdmitx_hdcp_Authenticate_Repeater(void)
{
    ...
    IT66121_HDCP_DEBUG_PRINTF(("Authentication for repeater\n"));
    ...
    {
        IT66121_HDCP_DEBUG_PRINTF(("HPD Before Fire Auth\n"));
        goto hdmitx_hdcp_Repeater_Fail ;
    }
    hdmitx_hdcp_Auth_Fire();
    //it66121_delay_ms(550); // emily add for test
    for(ii=0;ii<55;ii++)    //it66121_delay_ms(550); // emily add for test
    {
        if((B_TX_INT_HPD_PLUG|B_TX_INT_RX_SENSE)&HDMITX_ReadI2C_Byte(REG_TX_INT_STAT1))
        {
            goto hdmitx_hdcp_Repeater_Fail ;
    ...
    {
        IT66121_HDCP_DEBUG_PRINTF(("TimeOut = %d wait part 1\n",TimeOut));
        if((B_TX_INT_HPD_PLUG|B_TX_INT_RX_SENSE)&HDMITX_ReadI2C_Byte(REG_TX_INT_STAT1))
        {
            IT66121_HDCP_DEBUG_PRINTF(("HPD at wait part 1\n"));
            goto hdmitx_hdcp_Repeater_Fail ;
        }
        uc = HDMITX_ReadI2C_Byte(REG_TX_INT_STAT1);
        if(uc & B_TX_INT_DDC_BUS_HANG)
        {
            IT66121_HDCP_DEBUG_PRINTF(("DDC Bus hang\n"));
            goto hdmitx_hdcp_Repeater_Fail ;
        }
        ...
        if(uc & B_TX_INT_AUTH_FAIL)
        {
            ...
            HDMITX_WriteI2C_Byte(REG_TX_SYS_STATUS,0);
            */
            IT66121_HDCP_DEBUG_PRINTF(("hdmitx_hdcp_Authenticate_Repeater(): B_TX_INT_AUTH_FAIL.\n"));
            goto hdmitx_hdcp_Repeater_Fail ;
        }
        // emily add for test
        ...
        if(uc & B_TX_INT_KSVLIST_CHK)
        {
            ...
            HDMITX_WriteI2C_Byte(REG_TX_SYS_STATUS,B_TX_INTACTDONE);
            HDMITX_WriteI2C_Byte(REG_TX_SYS_STATUS,0);
            IT66121_HDCP_DEBUG_PRINTF(("B_TX_INT_KSVLIST_CHK\n"));
            break ;
        }
        it66121_delay_ms(5);
    }
    if(TimeOut == 0)
    {
        IT66121_HDCP_DEBUG_PRINTF(("Time out for wait KSV List checking interrupt\n"));
        goto hdmitx_hdcp_Repeater_Fail ;
    }
    ///////////////////////////////////////
    ...
    {
        IT66121_HDCP_DEBUG_PRINTF(("TimeOut=%d at wait FIFO ready\n",TimeOut));
        if((B_TX_INT_HPD_PLUG|B_TX_INT_RX_SENSE)&HDMITX_ReadI2C_Byte(REG_TX_INT_STAT1))
        {
            IT66121_HDCP_DEBUG_PRINTF(("HPD at wait FIFO ready\n"));
            goto hdmitx_hdcp_Repeater_Fail ;
        }
        if(hdmitx_hdcp_GetBCaps(&BCaps,&BStatus) == ER_FAIL)
        {
            IT66121_HDCP_DEBUG_PRINTF(("Get BCaps fail\n"));
            goto hdmitx_hdcp_Repeater_Fail ;
        }
        if(BCaps & B_TX_CAP_KSV_FIFO_RDY)
        {
             IT66121_HDCP_DEBUG_PRINTF(("FIFO Ready\n"));
             break ;
        }
        it66121_delay_ms(5);
    ...
    if(TimeOut == 0)
    {
        IT66121_HDCP_DEBUG_PRINTF(("Get KSV FIFO ready TimeOut\n"));
        goto hdmitx_hdcp_Repeater_Fail ;
    }
    IT66121_HDCP_DEBUG_PRINTF(("Wait timeout = %d\n",TimeOut));
    ...
    {
        IT66121_HDCP_DEBUG_PRINTF(("Invalid Down stream count,fail\n"));
        goto hdmitx_hdcp_Repeater_Fail ;
    }
#ifdef SUPPORT_SHA
...
//            IT66121_HDCP_DEBUG_PRINTF(("KSVFIFO[%d] = %02X %02X %02X %02X %02X is revoked\n",i,(int)KSVList[i*5],(int)KSVList[i*5+1],(int)KSVList[i*5+2],(int)KSVList[i*5+3],(int)KSVList[i*5+4]));
    ...
    if((B_TX_INT_HPD_PLUG|B_TX_INT_RX_SENSE)&HDMITX_ReadI2C_Byte(REG_TX_INT_STAT1))
    {
        IT66121_HDCP_DEBUG_PRINTF(("HPD at Final\n"));
        goto hdmitx_hdcp_Repeater_Fail ;
    }
#endif // SUPPORT_SHA
...
void hdmitx_hdcp_ResumeAuthentication(BYTE dev)
{
    setHDMITX_AVMute(TRUE);
    if(hdmitx_hdcp_Authenticate(dev) == ER_SUCCESS)
    {
    }
