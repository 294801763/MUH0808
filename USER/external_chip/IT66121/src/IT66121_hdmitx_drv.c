...
_CODE RegSetEntry HDMITX_DefaultAVIInfo_Table[] = {
    ...
    {0x63, 0xFF, 0x38},//MAX swing shenzy
...
BYTE AudioDelayCnt[HDMITX_MAX_DEV_COUNT]={
    0,
...
BYTE LastRefaudfreqnum[HDMITX_MAX_DEV_COUNT]={
    0,
#if (HDMITX_MAX_DEV_COUNT>=2)
    0,
#endif
...
BOOL bForceCTS[HDMITX_MAX_DEV_COUNT] = {
    FALSE,
#if (HDMITX_MAX_DEV_COUNT>=2)
    FALSE,
#endif
...
static BOOL AudioOutStatus[HDMITX_MAX_DEV_COUNT]={
    FALSE,
#if (HDMITX_MAX_DEV_COUNT>=2)
    FALSE,
#endif
...
void HDMITX_InitTxDev(BYTE dev,HDMITXDEV *pInstance)
{
	hdmiTxDev[dev] = *pInstance ;
}


#if 0
void IT66121_config_non_seq(unsigned char  non_seq)
{
	BYTE j = 0;

	while(HDMITX_Init_Table[j].offset != 0)
	{
		if(HDMITX_Init_Table[j].offset == 0x71)
		{
			HDMITX_Init_Table[j].OrMask = (non_seq==1)?0x1C:0x18;
			break;
		}
		j++;
	}
...
void InitHDMITX(BYTE dev)
{

    hdmitx_LoadRegSetting((RegSetEntry*)HDMITX_Init_Table);
    //HDMITX_WriteI2C_Byte(REG_TX_INT_CTRL,hdmiTxDev[0].bIntType);
    hdmiTxDev[dev].bIntPOL = (hdmiTxDev[dev].bIntType&B_TX_INTPOL_ACTH)?TRUE:FALSE ;
    ...
    hdmiTxDev[dev].RCLK = CalcRCLK();
#endif
    hdmitx_LoadRegSetting((RegSetEntry*)HDMITX_DefaultVideo_Table);
    if(hdmiTxDev[dev].bHDMIMode==1)
    {
    	 hdmitx_LoadRegSetting((RegSetEntry*)HDMITX_SetHDMI_Table);
	  }
		else
	  {
		   hdmitx_LoadRegSetting((RegSetEntry*)HDMITX_SetDVI_Table);
	  }

    hdmitx_LoadRegSetting((RegSetEntry*)HDMITX_DefaultAVIInfo_Table);
     if(hdmiTxDev[dev].bHDMIMode==1)hdmitx_LoadRegSetting((RegSetEntry*)HDMITX_DeaultAudioInfo_Table);
     if(hdmiTxDev[dev].bHDMIMode==1)hdmitx_LoadRegSetting((RegSetEntry*)HDMITX_Aud_CHStatus_LPCM_20bit_48Khz);
     if(hdmiTxDev[dev].bHDMIMode==1)hdmitx_LoadRegSetting((RegSetEntry*)HDMITX_AUD_SPDIF_2ch_24bit);
...
BOOL getHDMITX_LinkStatus(void)
{
    if(B_TX_RXSENDETECT & HDMITX_ReadI2C_Byte(REG_TX_SYS_STATUS))
    {
        ...
        {
            //IT66121_HDMITX_DEBUG_PRINTF(("getHDMITX_LinkStatus()!!\n") );
            return TRUE;
        }
    }
    //IT66121_HDMITX_DEBUG_PRINTF(("GetTMDS not Ready()!!\n") );

    return FALSE;
}

BYTE CheckHDMITX(BYTE dev ,BYTE *pHPD,BYTE *pHPDChange)
{
    BYTE intdata1,intdata2,intdata3,sysstat;
    BYTE  intclr3 = 0 ;
    BYTE PrevHPD = hdmiTxDev[dev].bHPD ;
    BYTE HPD ;
    sysstat = HDMITX_ReadI2C_Byte(REG_TX_SYS_STATUS);
	// IT66121_HDMITX_DEBUG_PRINTF(("REG_TX_SYS_STATUS = %X \n",sysstat));
    ...
    if(HPD==FALSE)
    {
        hdmiTxDev[dev].bAuthenticated = FALSE ;
    }
    if(sysstat & B_TX_INT_ACTIVE)
    {
		IT66121_HDMITX_DEBUG_PRINTF(("REG_TX_SYS_STATUS = 0x%02X \n",(int)sysstat));

        intdata1 = HDMITX_ReadI2C_Byte(REG_TX_INT_STAT1);
        IT66121_HDMITX_DEBUG_PRINTF(("INT_Handler: reg%X = %X\n",(int)REG_TX_INT_STAT1,(int)intdata1));
        if(intdata1 & B_TX_INT_AUD_OVERFLOW)
        {
            IT66121_HDMITX_DEBUG_PRINTF(("B_TX_INT_AUD_OVERFLOW.\n"));
		...
		{
		    IT66121_HDMITX_DEBUG_PRINTF(("DDC FIFO Error.\n"));
		    hdmitx_ClearDDCFIFO();
		    hdmiTxDev[dev].bAuthenticated= FALSE ;
		}
		if(intdata1 & B_TX_INT_DDC_BUS_HANG)
		{
		    IT66121_HDMITX_DEBUG_PRINTF(("DDC BUS HANG.\n"));
            hdmitx_AbortDDC();

            if(hdmiTxDev[dev].bAuthenticated)
            {
                IT66121_HDMITX_DEBUG_PRINTF(("when DDC hang,and aborted DDC,the HDCP authentication need to restart.\n"));
                #ifdef SUPPORT_HDCP
                hdmitx_hdcp_ResumeAuthentication(BYTE dev);
                #endif
            }
		}
		...
		{
            IT66121_HDMITX_DEBUG_PRINTF(("interrupt Authenticate Done.\n"));
		...
		{
		    hdmiTxDev[dev].bAuthenticated = FALSE;
            //IT66121_HDMITX_DEBUG_PRINTF(("interrupt Authenticate Fail.\n"));
		...
		*/
        ...
        if( intdata3 )
        {
            ...
            IT66121_HDMITX_DEBUG_PRINTF(("%s%s%s%s%s%s%s\n",
    ...
    hdmiTxDev[dev].bHPD = HPD ;

    UNUSED(intdata2);
    return HPD ;
}

void IT66121_HPD_Status(BYTE *pHPD)
{
	BYTE sysstat;
	...
	if(pHPD != NULL)
	{
				*pHPD = ((sysstat & (B_TX_HPDETECT)) == (B_TX_HPDETECT))?TRUE:FALSE;
	}
}

void HDMITX_PowerOn(void)
{
    hdmitx_LoadRegSetting((RegSetEntry*)HDMITX_PwrOn_Table);
}

void HDMITX_PowerDown(void)
{
    hdmitx_LoadRegSetting((RegSetEntry*)HDMITX_PwrDown_Table);
}
...
void hdmitx_LoadRegSetting(RegSetEntry table[])
{
    ...
    for( i = 0 ;  ; i++ )
    {
        ...
        else if( table[i].invAndMask == 0 && table[i].OrMask == 0 )
        {
            ...
            it66121_delay_ms(table[i].offset);
        }
...
SYS_STATUS getHDMITX_EDIDBytes(BYTE *pData,BYTE bSegment,BYTE offset,SHORT Count)
{
    ...
    // IT66121_HDMITX_DEBUG_PRINTF(("getHDMITX_EDIDBytes(%08lX,%d,%d,%d)\n",(ULONG)pData,(int)bSegment,(int)offset,(int)Count));
    if(!pData)
    {
//        IT66121_HDMITX_DEBUG_PRINTF(("getHDMITX_EDIDBytes(): Invallid pData pointer %08lX\n",(ULONG)pData));
        return ER_FAIL ;
    }
    if(HDMITX_ReadI2C_Byte(REG_TX_INT_STAT1) & B_TX_INT_DDC_BUS_HANG)
    {
        IT66121_HDMITX_DEBUG_PRINTF(("Called hdmitx_AboutDDC()\n"));
        hdmitx_AbortDDC();
    ...
    while(RemainedCount > 0)
    {
        ...
        IT66121_HDMITX_DEBUG_PRINTF(("getHDMITX_EDIDBytes(): ReqCount = %d,bCurrOffset = %d\n",(int)ReqCount,(int)bCurrOffset));
        ...
        for(TimeOut = 250 ; TimeOut > 0 ; TimeOut --)
        {
            ...
            {
                IT66121_HDMITX_DEBUG_PRINTF(("getHDMITX_EDIDBytes(): DDC_STATUS = %02X,fail.\n",(int)ucdata));
        ...
        if(TimeOut == 0)
        {
            IT66121_HDMITX_DEBUG_PRINTF(("getHDMITX_EDIDBytes(): DDC TimeOut. \n",(int)ucdata));
            // HDMITX_AndReg_Byte(REG_TX_INT_CTRL,~(1<<1));
            return ER_FAIL ;
        }
        do
...
void hdmitx_AbortDDC(void)
{
    ...
    for( i = 0 ; i < 2 ; i++ )
    {
        ...
        for( timeout = 0 ; timeout < 200 ; timeout++ )
        {
            ...
            {
//                IT66121_HDMITX_DEBUG_PRINTF(("hdmitx_AbortDDC Fail by reg16=%02X\n",(int)uc));
                break ;
            }
            it66121_delay_ms(1); // delay 1 ms to stable.
        }
    }
...
//////////////////////////////////////////////////////////////////////
        ...
        extern _CODE BYTE bCSCMtx_RGB2YUV_ITU601_16_235_IT66121[] ;
        extern _CODE BYTE bCSCMtx_RGB2YUV_ITU601_0_255_IT66121[] ;
        extern _CODE BYTE bCSCMtx_RGB2YUV_ITU709_16_235_IT66121[] ;
        extern _CODE BYTE bCSCMtx_RGB2YUV_ITU709_0_255_IT66121[] ;
    #endif

    #if (defined (SUPPORT_OUTPUTRGB)) && (defined (SUPPORT_INPUTYUV))
        extern _CODE BYTE bCSCMtx_YUV2RGB_ITU601_16_235_IT66121[] ;
        extern _CODE BYTE bCSCMtx_YUV2RGB_ITU601_0_255_IT66121[] ;
        extern _CODE BYTE bCSCMtx_YUV2RGB_ITU709_16_235_IT66121[] ;
        extern _CODE BYTE bCSCMtx_YUV2RGB_ITU709_0_255_IT66121[] ;
...
BOOL HDMITX_EnableVideoOutput(BYTE dev,VIDEOPCLKLEVEL level,BYTE inputColorMode,BYTE outputColorMode,BYTE bHDMI)
{
    ...
    hdmiTxDev[dev].bHDMIMode = (BYTE)bHDMI ;
    ...
    hdmitx_SetInputMode(inputColorMode,hdmiTxDev[dev].bInputVideoSignalType);
...
//////////////////////////////////////////////////////////////////////
BOOL setHDMITX_VideoSignalType(BYTE dev,BYTE inputSignalType)
{
	hdmiTxDev[dev].bInputVideoSignalType = inputSignalType ;
...
void setHDMITX_ColorDepthPhase(BYTE dev, BYTE ColorDepth,BYTE bPhase)
{
#ifdef IT6615
    BYTE uc ;
    BYTE bColorDepth ;
    ...
    if(ColorDepth == 30)
    {
        ...
        IT66121_HDMITX_DEBUG_PRINTF(("bColorDepth = B_TX_CD_30\n"));
    }
    else if (ColorDepth == 36)
    {
        bColorDepth = B_TX_CD_36 ;
        IT66121_HDMITX_DEBUG_PRINTF(("bColorDepth = B_TX_CD_36\n"));
    }
    /*
    ...
    */
	...
	IT66121_HDMITX_DEBUG_PRINTF(("setHDMITX_ColorDepthPhase(%02X), regC1 = %02X\n",(int)bColorDepth,(int)HDMITX_ReadI2C_Byte(REG_TX_GCP))) ;
#endif
}
...
BOOL setHDMITX_SyncEmbeddedByVIC(BYTE VIC,BYTE bInputType)
{
    ...
    IT66121_HDMITX_DEBUG_PRINTF(("setHDMITX_SyncEmbeddedByVIC(%d,%x)\n",(int)VIC,(int)bInputType));
    if( VIC > 0 )
    {
        for(i=0;i< MaxIndex;i ++)
        {
            ...
            {
                fmt_index=i;
                IT66121_HDMITX_DEBUG_PRINTF(("fmt_index=%02x)\n",(int)fmt_index));
                IT66121_HDMITX_DEBUG_PRINTF(("***Fine Match Table ***\n"));
                break;
            }
        }
    }
    ...
    {
        IT66121_HDMITX_DEBUG_PRINTF(("***No Match VIC == 0 ***\n"));
        return FALSE ;
    }
    ...
    {
        //return FALSE;
        IT66121_HDMITX_DEBUG_PRINTF(("***No Match VIC ***\n"));
        return FALSE ;
    }
...
void hdmitx_SetInputMode(BYTE InputColorMode,BYTE bInputSignalType)
{
    ...
    {
        ucData |= B_TX_PCLKDIV2 ; IT66121_HDMITX_DEBUG_PRINTF(("PCLK Divided by 2 mode\n"));
    }
    if(bInputSignalType & T_MODE_CCIR656)
    {
        ucData |= B_TX_2X656CLK ; IT66121_HDMITX_DEBUG_PRINTF(("CCIR656 mode\n"));
    }
    if(bInputSignalType & T_MODE_SYNCEMB)
    {
        ucData |= B_TX_SYNCEMB ; IT66121_HDMITX_DEBUG_PRINTF(("Sync Embedded mode\n"));
    }
    if(bInputSignalType & T_MODE_INDDR)
    {
        ucData |= B_TX_INDDR ; IT66121_HDMITX_DEBUG_PRINTF(("Input DDR mode\n"));
    }
...
void hdmitx_SetCSCScale(BYTE bInputMode,BYTE bOutputMode)
{
    ...
    IT66121_HDMITX_DEBUG_PRINTF(("hdmitx_SetCSCScale(BYTE bInputMode = %x,BYTE bOutputMode = %x)\n", (int)bInputMode, (int)bOutputMode)) ;
    switch(bInputMode&F_MODE_CLRMOD_MASK)
    {
    #ifdef SUPPORT_INPUTYUV444
    case F_MODE_YUV444:
        IT66121_HDMITX_DEBUG_PRINTF(("Input mode is YUV444 "));
        switch(bOutputMode&F_MODE_CLRMOD_MASK)
        {
        case F_MODE_YUV444:
            IT66121_HDMITX_DEBUG_PRINTF(("Output mode is YUV444\n"));
            csc = B_HDMITX_CSC_BYPASS ;
            break ;

        case F_MODE_YUV422:
            IT66121_HDMITX_DEBUG_PRINTF(("Output mode is YUV422\n"));
            ...
            break ;
        case F_MODE_RGB444:
            IT66121_HDMITX_DEBUG_PRINTF(("Output mode is RGB24\n"));
    ...
    case F_MODE_YUV422:
        IT66121_HDMITX_DEBUG_PRINTF(("Input mode is YUV422\n"));
        switch(bOutputMode&F_MODE_CLRMOD_MASK)
        {
        case F_MODE_YUV444:
            IT66121_HDMITX_DEBUG_PRINTF(("Output mode is YUV444\n"));
    ...
    case F_MODE_RGB444:
        IT66121_HDMITX_DEBUG_PRINTF(("Input mode is RGB24\n"));
        switch(bOutputMode&F_MODE_CLRMOD_MASK)
        {
        case F_MODE_YUV444:
            IT66121_HDMITX_DEBUG_PRINTF(("Output mode is YUV444\n"));
    ...
    {
        IT66121_HDMITX_DEBUG_PRINTF(("CSC = RGB2YUV %x ",csc));
        switch(bInputMode&(F_VIDMODE_ITU709|F_VIDMODE_16_235))
        {
        case F_VIDMODE_ITU709|F_VIDMODE_16_235:
            IT66121_HDMITX_DEBUG_PRINTF(("ITU709 16-235 "));
            for( i = 0 ; i < IT66121_SIZEOF_CSCMTX ; i++ ){ HDMITX_WriteI2C_Byte(REG_TX_CSC_YOFF+i,bCSCMtx_RGB2YUV_ITU709_16_235_IT66121[i]) ; IT66121_HDMITX_DEBUG_PRINTF(("reg%02X <- %02X\n",(int)(i+REG_TX_CSC_YOFF),(int)bCSCMtx_RGB2YUV_ITU709_16_235_IT66121[i]));}
            break ;
        case F_VIDMODE_ITU709|F_VIDMODE_0_255:
            IT66121_HDMITX_DEBUG_PRINTF(("ITU709 0-255 "));
            for( i = 0 ; i < IT66121_SIZEOF_CSCMTX ; i++ ){ HDMITX_WriteI2C_Byte(REG_TX_CSC_YOFF+i,bCSCMtx_RGB2YUV_ITU709_0_255_IT66121[i]) ; IT66121_HDMITX_DEBUG_PRINTF(("reg%02X <- %02X\n",(int)(i+REG_TX_CSC_YOFF),(int)bCSCMtx_RGB2YUV_ITU709_0_255_IT66121[i]));}
            break ;
        case F_VIDMODE_ITU601|F_VIDMODE_16_235:
            IT66121_HDMITX_DEBUG_PRINTF(("ITU601 16-235 "));
            for( i = 0 ; i < IT66121_SIZEOF_CSCMTX ; i++ ){ HDMITX_WriteI2C_Byte(REG_TX_CSC_YOFF+i,bCSCMtx_RGB2YUV_ITU601_16_235_IT66121[i]) ; IT66121_HDMITX_DEBUG_PRINTF(("reg%02X <- %02X\n",(int)(i+REG_TX_CSC_YOFF),(int)bCSCMtx_RGB2YUV_ITU601_16_235_IT66121[i]));}
            break ;
        case F_VIDMODE_ITU601|F_VIDMODE_0_255:
        default:
            IT66121_HDMITX_DEBUG_PRINTF(("ITU601 0-255 "));
            for( i = 0 ; i < IT66121_SIZEOF_CSCMTX ; i++ ){ HDMITX_WriteI2C_Byte(REG_TX_CSC_YOFF+i,bCSCMtx_RGB2YUV_ITU601_0_255_IT66121[i]) ; IT66121_HDMITX_DEBUG_PRINTF(("reg%02X <- %02X\n",(int)(i+REG_TX_CSC_YOFF),(int)bCSCMtx_RGB2YUV_ITU601_0_255_IT66121[i]));}
            break ;
        }
    }
    #endif
    ...
    {
        IT66121_HDMITX_DEBUG_PRINTF(("CSC = YUV2RGB %x ",csc));

        switch(bInputMode&(F_VIDMODE_ITU709|F_VIDMODE_16_235))
        {
        case F_VIDMODE_ITU709|F_VIDMODE_16_235:
            IT66121_HDMITX_DEBUG_PRINTF(("ITU709 16-235 "));
            for( i = 0 ; i < IT66121_SIZEOF_CSCMTX ; i++ ){ HDMITX_WriteI2C_Byte(REG_TX_CSC_YOFF+i,bCSCMtx_YUV2RGB_ITU709_16_235_IT66121[i]) ; IT66121_HDMITX_DEBUG_PRINTF(("reg%02X <- %02X\n",(int)(i+REG_TX_CSC_YOFF),(int)bCSCMtx_YUV2RGB_ITU709_16_235_IT66121[i]));}
            break ;
        case F_VIDMODE_ITU709|F_VIDMODE_0_255:
            IT66121_HDMITX_DEBUG_PRINTF(("ITU709 0-255 "));
            for( i = 0 ; i < IT66121_SIZEOF_CSCMTX ; i++ ){ HDMITX_WriteI2C_Byte(REG_TX_CSC_YOFF+i,bCSCMtx_YUV2RGB_ITU709_0_255_IT66121[i]) ; IT66121_HDMITX_DEBUG_PRINTF(("reg%02X <- %02X\n",(int)(i+REG_TX_CSC_YOFF),(int)bCSCMtx_YUV2RGB_ITU709_0_255_IT66121[i]));}
            break ;
        case F_VIDMODE_ITU601|F_VIDMODE_16_235:
            IT66121_HDMITX_DEBUG_PRINTF(("ITU601 16-235 "));
            for( i = 0 ; i < IT66121_SIZEOF_CSCMTX ; i++ ){ HDMITX_WriteI2C_Byte(REG_TX_CSC_YOFF+i,bCSCMtx_YUV2RGB_ITU601_16_235_IT66121[i]) ; IT66121_HDMITX_DEBUG_PRINTF(("reg%02X <- %02X\n",(int)(i+REG_TX_CSC_YOFF),(int)bCSCMtx_YUV2RGB_ITU601_16_235_IT66121[i]));}
            break ;
        case F_VIDMODE_ITU601|F_VIDMODE_0_255:
        default:
            IT66121_HDMITX_DEBUG_PRINTF(("ITU601 0-255 "));
            for( i = 0 ; i < IT66121_SIZEOF_CSCMTX ; i++ ){ HDMITX_WriteI2C_Byte(REG_TX_CSC_YOFF+i,bCSCMtx_YUV2RGB_ITU601_0_255_IT66121[i]) ; IT66121_HDMITX_DEBUG_PRINTF(("reg%02X <- %02X\n",(int)(i+REG_TX_CSC_YOFF),(int)bCSCMtx_YUV2RGB_ITU601_0_255_IT66121[i]));}
            break ;
        }
    }
    #endif
...
void hdmitx_SetupAFE(VIDEOPCLKLEVEL level)
{
    ...
    switch(level)
    {
        case PCLK_HIGH:
            ...
            IT66121_HDMITX_DEBUG_PRINTF(("hdmitx_SetupAFE()===================HIGHT\n"));
            break ;
        default:
            HDMITX_SetI2C_Byte(0x62, 0x90, 0x10);
            HDMITX_SetI2C_Byte(0x64, 0x89, 0x09);
            HDMITX_SetI2C_Byte(0x68, 0x10, 0x10);
            IT66121_HDMITX_DEBUG_PRINTF(("hdmitx_SetupAFE()===================LOW\n"));
            break ;
    }
	#if Debug_message
	reg62 = HDMITX_ReadI2C_Byte(0x62);
	reg64 = HDMITX_ReadI2C_Byte(0x64);
	reg68 = HDMITX_ReadI2C_Byte(0x68);

	IT66121_HDMITX_DEBUG_PRINTF(("\r\n\r\n**************\r\n"));
	IT66121_HDMITX_DEBUG_PRINTF(("AFE Reg 0x62=0x%2x \n",reg62));
	IT66121_HDMITX_DEBUG_PRINTF(("AFE Reg 0x64=0x%2x \n",reg64));
	IT66121_HDMITX_DEBUG_PRINTF(("AFE Reg 0x68=0x%2x \n",reg68));
    ...
    HDMITX_SetI2C_Byte(0x64, 0x01, 0x00);//调整滤波器的参数 shenzy
...
void setHDMITX_LPCMAudio(BYTE dev,BYTE AudioSrcNum, BYTE AudSWL, BOOL bSPDIF)
{

    BYTE AudioEnable, AudioFormat ;

    AudioEnable = 0 ;
    AudioFormat = hdmiTxDev[dev].bOutputAudioMode ;
    ...
    hdmiTxDev[dev].bAudioChannelEnable=AudioEnable;
...
void setHDMITX_HBRAudio(BYTE dev,BOOL bSPDIF)
{
    // BYTE rst;
    Switch_HDMITX_Bank(0);
    ...
    hdmiTxDev[dev].bAudioChannelEnable=HDMITX_ReadI2C_Byte(REG_TX_AUDIO_CTRL0);
...
void HDMITX_DisableAudioOutput(BYTE dev)
{
    //BYTE uc = (HDMITX_ReadI2C_Byte(REG_TX_SW_RST) | (B_HDMITX_AUD_RST | B_TX_AREF_RST));
    //HDMITX_WriteI2C_Byte(REG_TX_SW_RST,uc);
    AudioDelayCnt[dev]=AudioOutDelayCnt;
    LastRefaudfreqnum[dev]=0;
...
void HDMITX_EnableAudioOutput(BYTE dev,BYTE AudioType, BOOL bSPDIF,  ULONG SampleFreq,  BYTE ChNum, BYTE *pIEC60958ChStat, ULONG TMDSClock)
{
    static _IDATA BYTE ucIEC60958ChStat[5] ;

    BYTE Fs ;
    AudioDelayCnt[dev]=36;
    LastRefaudfreqnum[dev]=0;
    hdmiTxDev[dev].TMDSClock=TMDSClock;
    hdmiTxDev[dev].bAudioChannelEnable=0;
    hdmiTxDev[dev].bSPDIF_OUT=bSPDIF;
    ...
    if( AudioType != T_AUDIO_DSD)
    {
        ...
        hdmiTxDev[dev].bAudFs=AUDFS_OTHER;
    #else
        hdmiTxDev[dev].bAudFs=Fs;
    #endif
        setHDMITX_NCTS(dev,hdmiTxDev[dev].bAudFs);
    ...
    {
    case T_AUDIO_HBR:
        IT66121_HDMITX_DEBUG_PRINTF(("T_AUDIO_HBR\n"));
        ...
        setHDMITX_HBRAudio(dev,bSPDIF);

        break ;
    case T_AUDIO_DSD:
        IT66121_HDMITX_DEBUG_PRINTF(("T_AUDIO_DSD\n"));
        setHDMITX_DSDAudio();
        break ;
    case T_AUDIO_NLPCM:
        IT66121_HDMITX_DEBUG_PRINTF(("T_AUDIO_NLPCM\n"));
        pIEC60958ChStat[0] |= 1<<1 ;
        setHDMITX_ChStat(pIEC60958ChStat);
        setHDMITX_NLPCMAudio(bSPDIF);
        break ;
    case T_AUDIO_LPCM:
        IT66121_HDMITX_DEBUG_PRINTF(("T_AUDIO_LPCM\n"));
        pIEC60958ChStat[0] &= ~(1<<1);

        setHDMITX_ChStat(pIEC60958ChStat);
        setHDMITX_LPCMAudio(dev,(ChNum+1)/2, SUPPORT_AUDI_AudSWL, bSPDIF);
        // can add auto adjust
        break ;
    }
    HDMITX_AndReg_Byte(REG_TX_INT_MASK1,(~B_TX_AUDIO_OVFLW_MASK));
    HDMITX_WriteI2C_Byte(REG_TX_AUDIO_CTRL0, hdmiTxDev[dev].bAudioChannelEnable);

    HDMITX_SetI2C_Byte(REG_TX_SW_RST,(B_HDMITX_AUD_RST|B_TX_AREF_RST),0);
}

void hdmitx_AutoAdjustAudio(BYTE dev)
{
    ...
    if( aCTS == 0)
    {
        IT66121_HDMITX_DEBUG_PRINTF(("aCTS== 0"));
        return;
    }
    uc = HDMITX_ReadI2C_Byte(REG_TX_GCP);

    cTMDSClock = hdmiTxDev[dev].TMDSClock ;
    //TMDSClock=GetInputPclk();
    IT66121_HDMITX_DEBUG_PRINTF(("PCLK = %u0,000\n",(WORD)(cTMDSClock/10000)));
    switch(uc & 0x70)
    {
    case 0x50:
        cTMDSClock *= 5 ;
    ...
    IT66121_HDMITX_DEBUG_PRINTF(("SampleFreq = %u0\n",(WORD)(SampleFreq/10)));
    ...
    {
        fs = AUDFS_OTHER;
        IT66121_HDMITX_DEBUG_PRINTF(("fs = AUDFS_OTHER\n"));
    }
    if(hdmiTxDev[dev].bAudFs != fs)
    {
        hdmiTxDev[dev].bAudFs=fs;
        setHDMITX_NCTS(dev,hdmiTxDev[dev].bAudFs); // set N, CTS by new generated clock.
        //CurrCTS=0;
        return;
    }
    return;
...
BOOL hdmitx_IsAudioChang(BYTE dev)
{
    //ULONG pCTS=0;
    ...
    //IT66121_HDMITX_DEBUG_PRINTF(("Refaudfreqnum=%X    pCTS= %u",(WORD)Refaudfreqnum,(WORD)(pCTS/10000)));
    //if((pCTS%10000)<1000)IT66121_HDMITX_DEBUG_PRINTF(("0"));
    //if((pCTS%10000)<100)IT66121_HDMITX_DEBUG_PRINTF(("0"));
    //if((pCTS%10000)<10)IT66121_HDMITX_DEBUG_PRINTF(("0"));
    //IT66121_HDMITX_DEBUG_PRINTF(("%u\n",(WORD)(pCTS%10000)));
    if((1<<4)&HDMITX_ReadI2C_Byte(0x5f))
    {
        //dbg_printf("=======XXXXXXXXXXX=========\n");
        return FALSE;
    }
    if(LastRefaudfreqnum[dev]>Refaudfreqnum)
        {FreDiff=LastRefaudfreqnum[dev]-Refaudfreqnum;}
    else
        {FreDiff=Refaudfreqnum-LastRefaudfreqnum[dev];}
    LastRefaudfreqnum[dev]=Refaudfreqnum;
    if(3<FreDiff)
    {
        IT66121_HDMITX_DEBUG_PRINTF(("Aduio FreDiff=%d\n",(int)FreDiff));
...
void setHDMITX_AudioChannelEnable(BYTE dev,BOOL EnableAudio_b)
{


    if(EnableAudio_b)
    {
        if(AudioDelayCnt[dev]==0)
        {
            ...
            {
                hdmitx_AutoAdjustAudio(dev);
        #else
            if(AudioOutStatus[dev]==FALSE)
            {
                setHDMITX_NCTS(dev,hdmiTxDev[dev].bAudFs);
        #endif
                ...
                dbg_printf("Audio Out Enable\n");
        #ifndef SUPPORT_AUDIO_MONITOR
                AudioOutStatus[dev]=TRUE;
        #endif
            }
        }
        else
        {
            AudioOutStatus[dev]=FALSE;
...
void setHDMITX_NCTS(BYTE dev,BYTE Fs)
{
    ULONG n;
        ...
        while(LoopCnt--)
        {
            ...
            else
            {
                if(LastCTS>CTS )
                    ...
                    {diff=CTS-LastCTS;}
                //IT66121_HDMITX_DEBUG_PRINTF(("LastCTS= %u%u",(WORD)(LastCTS/10000),(WORD)(LastCTS%10000)));
                //IT66121_HDMITX_DEBUG_PRINTF(("       CTS= %u%u\n",(WORD)(CTS/10000),(WORD)(CTS%10000)));
                LastCTS=CTS;
                if(5>diff)
                {
                    CTSStableCnt++;
...
SYS_STATUS hdmitx_SetAVIInfoFrame(AVI_InfoFrame *pAVIInfoFrame)
{
    ...
    /*
    IT66121_HDMITX_DEBUG_PRINTF(("SetAVIInfo(): "));
    IT66121_HDMITX_DEBUG_PRINTF(("%02X ",(int)HDMITX_ReadI2C_Byte(REG_TX_AVIINFO_DB1)));
    IT66121_HDMITX_DEBUG_PRINTF(("%02X ",(int)HDMITX_ReadI2C_Byte(REG_TX_AVIINFO_DB2)));
    IT66121_HDMITX_DEBUG_PRINTF(("%02X ",(int)HDMITX_ReadI2C_Byte(REG_TX_AVIINFO_DB3)));
    IT66121_HDMITX_DEBUG_PRINTF(("%02X ",(int)HDMITX_ReadI2C_Byte(REG_TX_AVIINFO_DB4)));
    IT66121_HDMITX_DEBUG_PRINTF(("%02X ",(int)HDMITX_ReadI2C_Byte(REG_TX_AVIINFO_DB5)));
    IT66121_HDMITX_DEBUG_PRINTF(("%02X ",(int)HDMITX_ReadI2C_Byte(REG_TX_AVIINFO_DB6)));
    IT66121_HDMITX_DEBUG_PRINTF(("%02X ",(int)HDMITX_ReadI2C_Byte(REG_TX_AVIINFO_DB7)));
    IT66121_HDMITX_DEBUG_PRINTF(("%02X ",(int)HDMITX_ReadI2C_Byte(REG_TX_AVIINFO_DB8)));
    IT66121_HDMITX_DEBUG_PRINTF(("%02X ",(int)HDMITX_ReadI2C_Byte(REG_TX_AVIINFO_DB9)));
    IT66121_HDMITX_DEBUG_PRINTF(("%02X ",(int)HDMITX_ReadI2C_Byte(REG_TX_AVIINFO_DB10)));
    IT66121_HDMITX_DEBUG_PRINTF(("%02X ",(int)HDMITX_ReadI2C_Byte(REG_TX_AVIINFO_DB11)));
    IT66121_HDMITX_DEBUG_PRINTF(("%02X ",(int)HDMITX_ReadI2C_Byte(REG_TX_AVIINFO_DB12)));
    IT66121_HDMITX_DEBUG_PRINTF(("%02X ",(int)HDMITX_ReadI2C_Byte(REG_TX_AVIINFO_DB13)));
    IT66121_HDMITX_DEBUG_PRINTF(("\n"));
    */
...
void DumpHDMITXReg()
{
    ...
    for(j = 0 ; j < 16 ; j++)
    {
        IT66121_HDMITX_DEBUG_PRINTF((" %02X",(int)j));
        if((j == 3)||(j==7)||(j==11))
        {
            IT66121_HDMITX_DEBUG_PRINTF(("  "));
        }
    ...
    {
        IT66121_HDMITX_DEBUG_PRINTF(("[%3X]  ",i));
        for(j = 0 ; j < 16 ; j++)
        {
            if( (i+j)!= 0x17)
            {
                ucData = HDMITX_ReadI2C_Byte((BYTE)((i+j)&0xFF));
                IT66121_HDMITX_DEBUG_PRINTF((" %02X",(int)ucData));
            }
            else
            {
                IT66121_HDMITX_DEBUG_PRINTF((" XX",(int)ucData)); // for DDC FIFO
            }
            if((j == 3)||(j==7)||(j==11))
            {
                IT66121_HDMITX_DEBUG_PRINTF((" -"));
