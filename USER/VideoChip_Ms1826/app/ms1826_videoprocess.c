...
extern VOID ms1826_hal_hdmi_rx_audio_hbr_pll_cfg(UINT8 u8_rx_idx, BOOL b_is_hbr_aud_input);
extern VOID ms1826_hal_hdmi_rx_audio_fifo_reset(UINT8 u8_rx_idx, BOOL b_reset);
extern VOID ms1826_hal_hdmi_rx_hbr_en(UINT8 u8_rx_idx, BOOL b_is_hbr_aud_input);
...
static VOID _ms1826_init(UINT8 u8Dev)
{
    ...
    {
        st_pll_dev.u32_clk_out = 50000 * 10000;
        st_pll_dev.u8_pll_idx = PLLM;
        st_pll_dev.b_osc_input = MS_TRUE;
        api_pll_config(&st_pll_dev);
...
static VOID _video_port_hdmi_rx_init(UINT8 u8Dev)
{
    ...
    for(u8_rx_id = 0; u8_rx_id < 4; u8_rx_id ++)
    {
        ...
        {
            api_misc_timing_get(VFMT_CEA_16_1920x1080P_60HZ,&g_stVideo_HDMIRx_Timing[u8Dev][u8_rx_id]);
            api_misc_timing_get(VFMT_CEA_16_1920x1080P_60HZ,&g_st_hdmi_rx[u8Dev][u8_rx_id].st_rx_video_timing);
        }
        else
        {
            memset(&g_stVideo_HDMIRx_Timing[u8Dev][u8_rx_id],NULL,sizeof(HDMI_VIDEO_TIMING_T));
        }
        g_st_hdmi_rx[u8Dev][u8_rx_id].u8_rx_idx = g_u8_HDMI_RX_PROT[u8_rx_id];
        ...
        g_st_hdmi_rx[u8Dev][u8_rx_id].b_timing_stable = MS_FALSE;
...
VOID _video_timgen_cfg(UINT8 u8Dev,UINT8 u8_timgen_id)
{
    ...
    st_pll_dev.u8_pll_idx = PLLV1 + u8_timgen_id;
    st_pll_dev.b_osc_input = MS_TRUE;
    st_pll_dev.u32_clk_out = g_stVideo_HDMITx_Timing[u8Dev][u8_timgen_id].u16_pixclk * 10000;
    api_pll_config(&st_pll_dev);
 ...
 * Parameter: u8_rx_idx: select lock rx;
 *            u8_timgen_id: To lock timing generator
 * Return  : NONE
 * History :
 ***********************************************************************/
VOID _video_timgen_cfg_frame_lock(UINT8 u8Dev,UINT8 u8_rx_idx, UINT8 u8_timgen_id)
{
	MISC_MODULE_CLK_T st_tmg_src;
	PLL_DEV_T st_pll_dev;
	MISC_MODULE_CLK_T st_module_clk;

    LOG("Timgen %d lock rx %d\r\n",u8_timgen_id,u8_rx_idx);
	api_misc_timing_get(g_arrOutputTable[g_u8_output_index[u8Dev][u8_timgen_id]],&g_stVideo_HDMITx_Timing[u8Dev][u8_timgen_id]);

	st_module_clk.u8_clkdiv	= MISC_CLKDIV_1;
	st_module_clk.u8_dst_module	= MISC_VIDEO_MODULE_PLLV1 + u8_timgen_id ;
	st_module_clk.u8_src_clk = MISC_CLKSRC_RX1 + u8_rx_idx;		//freerun妯″紡閫夋嫨 MISC_CLKSRC_XTAL
	api_misc_pllv_clkin_sel(&st_module_clk);

	st_pll_dev.u8_pll_idx =	PLLV1 + u8_timgen_id;
	st_pll_dev.b_osc_input = MS_FALSE;				//freerun妯″紡閫夋嫨鏅舵尟杈撳叆 MS_TRUE
	st_pll_dev.u32_clk_in = g_stVideo_HDMIRx_Timing[u8Dev][u8_rx_idx].u16_pixclk * 10000;	//freerun妯″紡閫夋嫨鏅舵尟杈撳叆 鏃犻渶閰嶇疆锛岄粯璁や负24M
	st_pll_dev.u32_clk_out = g_stVideo_HDMITx_Timing[u8Dev][u8_timgen_id].u16_pixclk * 10000;
	api_pll_config(&st_pll_dev);
	...
	api_timgen_instance_init(&g_st_timgen[u8Dev][u8_timgen_id], u8_timgen_id);
	g_st_timgen[u8Dev][u8_timgen_id].u8_workmode =	Casecade_Mode;		//freerun妯″紡閫夋嫨 FreeRun_Mode
	g_st_timgen[u8Dev][u8_timgen_id].u8_timgen_id = u8_timgen_id;
	g_st_timgen[u8Dev][u8_timgen_id].b_timgen_en =	MS_ENABLE;
	g_st_timgen[u8Dev][u8_timgen_id].pst_timing = &g_stVideo_HDMITx_Timing[u8Dev][u8_timgen_id];
...
VOID _video_port_hdmi_tx_driver_config(UINT8 u8Dev,u8 u8_tx_id)
{
...
//    st_tx_driver_pre_emphasis.u8_txIndex = u8_tx_id;
...
//    api_hdmi_tx_set_driver_pre_emphasis(&st_tx_driver_pre_emphasis);
    ...
    st_tx_volt_level.u8_txIndex = u8_tx_id;
...
VOID _video_port_hdmi_tx_init(UINT8 u8Dev)
{
    ...
    {
        api_hdmi_tx_instance_init(&g_st_hdmi_tx[u8Dev][u8_tx_id], g_u8_HDMI_TX_PROT[u8_tx_id]);
        api_misc_timing_get(g_arrOutputTable[g_u8_output_index[u8Dev][u8_tx_id]],&g_stVideo_HDMITx_Timing[u8Dev][u8_tx_id]);
        g_st_hdmi_tx[u8Dev][u8_tx_id].u8_txIndex =  g_u8_HDMI_TX_PROT[u8_tx_id];
        g_st_hdmi_tx[u8Dev][u8_tx_id].b_is_hdmi = (BOOL)g_u8TxFormat[u8Dev][u8_tx_id];
        g_st_hdmi_tx[u8Dev][u8_tx_id].b_full_edid = FALSE;
        g_st_hdmi_tx[u8Dev][u8_tx_id].u8_color_depth =  HDMI_COLOR_DEPTH_24;
        g_st_hdmi_tx[u8Dev][u8_tx_id].u8_audio_mode = HDMI_AUDIO_STREAM_TYPE_MAS;
        g_st_hdmi_tx[u8Dev][u8_tx_id].u8_audio_src  = FROM_HDMI_RX_AUDIO;
        g_st_hdmi_tx[u8Dev][u8_tx_id].u8_video_clk_src  = FROM_RX_OR_PLL_PIXEL_CLK;
        g_st_hdmi_tx[u8Dev][u8_tx_id].u32_tmds_clk  = g_stVideo_HDMITx_Timing[u8Dev][u8_tx_id].u16_pixclk;
        g_st_hdmi_tx[u8Dev][u8_tx_id].pu8_edid_buf  = (VOID *)hdmi_tx_edid[u8Dev][u8_tx_id];
        g_st_hdmi_tx[u8Dev][u8_tx_id].pu8_bksv_buf  = (UINT8 *)u8_tx_ksv_buf;
        g_st_hdmi_tx[u8Dev][u8_tx_id].pu8_key_buf = (UINT8 *)u8_tx_key_buf;
        g_st_hdmi_tx[u8Dev][u8_tx_id].pst_edid_info = (VOID *)&g_hdmi_edid_flag[u8Dev][u8_tx_id];
        api_hdmi_tx_init(&g_st_hdmi_tx[u8Dev][u8_tx_id]);
        memcpy((&g_st_hdmi_tx[u8Dev][u8_tx_id].st_infoframes_packets), &gst_inf_pack[u8Dev], sizeof(HDMI_INFOFRAMES_PACKETS_T));
        g_st_hdmi_tx[u8Dev][u8_tx_id].st_infoframes_packets.st_avi_infoframe.u8_video_code = g_arrOutputTable[g_u8_output_index[u8Dev][u8_tx_id]];
//        api_hdmi_tx_hdcp14_init(&g_st_hdmi_tx[u8Dev][u8_tx_id]);
        ...
        _tx_csc_cfg(u8Dev,u8_tx_id);

        g_st_tx_mux[u8Dev][u8_tx_id].u8_tx_module = MISC_VIDEO_MODULE_TX1 + u8_tx_id;
        g_st_tx_mux[u8Dev][u8_tx_id].u8_video_mux = MISC_VIDEO_MODULE_MIX1  + u8_tx_id;
        api_misc_tx_mux_cfg(&g_st_tx_mux[u8Dev][u8_tx_id]);
        api_vehc_instance_init(&g_st_vovehc[u8Dev][u8_tx_id], u8_tx_id);
        app_bcsh_adjust(u8Dev,u8_tx_id);
        api_hdmi_tx_output_en(&g_st_hdmi_tx[u8Dev][u8_tx_id], MS_TRUE);
    }
...
VOID _digital_audio_mute(UINT8 u8Dev,UINT8 u8_path, BOOL b_mute)
{
    ...
    for(u8_tx_id = 0 ; u8_tx_id < 4 ; u8_tx_id ++)
    {
        if(Vipss_Mapping_Rx[u8Dev][u8_tx_id] == u8_path)
        {
            if(g_u8DigitalAudioSta[u8Dev][Vipss_Mapping_Rx[u8Dev][u8_tx_id]]!=((UINT8)b_mute))
            {
                g_u8DigitalAudioSta[u8Dev][Vipss_Mapping_Rx[u8Dev][u8_tx_id]] = ((UINT8)b_mute);
                if(b_mute)
                {
                    if(g_bDigitalAudioSwByHdmiTx[u8Dev][u8_tx_id])
                        g_stMs1826Event[u8Dev].u8TxDigitalAudioSource[u8_tx_id] = u8_Tx_audio_sel[u8Dev][u8_tx_id];
                    app_digital_audio_cfg(u8Dev,g_stMs1826Event[u8Dev].u8TxDigitalAudioSource[u8_tx_id], g_u8_i2s_idx[u8_tx_id], FALSE);
                    app_digital_audio_cfg(u8Dev,g_stMs1826Event[u8Dev].u8TxDigitalAudioSource[u8_tx_id], g_u8_spdif_idx[u8_tx_id], FALSE);
                }
                else
                {
                    if(g_bDigitalAudioSwByHdmiTx[u8Dev][u8_tx_id])
                        g_stMs1826Event[u8Dev].u8TxDigitalAudioSource[u8_tx_id] = u8_Tx_audio_sel[u8Dev][u8_tx_id];
                    app_digital_audio_cfg(u8Dev,g_stMs1826Event[u8Dev].u8TxDigitalAudioSource[u8_tx_id], g_u8_i2s_idx[u8_tx_id], b_digital_audio_mute[u8Dev][u8_tx_id]?FALSE:TRUE);
                    app_digital_audio_cfg(u8Dev,g_stMs1826Event[u8Dev].u8TxDigitalAudioSource[u8_tx_id], g_u8_spdif_idx[u8_tx_id], b_digital_audio_mute[u8Dev][u8_tx_id]?FALSE:TRUE);
                }
            }
        }
    }
...
VOID _video_mute(UINT8 u8Dev,UINT8 u8_path, BOOL b_mute)
{
    UINT8 u8_tx_id;
//    LOG("_video_mute[%d] sta:%d\r\n",u8_path,b_mute);
    if(g_u8_display_mode[u8Dev]== MS1826_SEAMLESS_MATRIX_SWITCHER_OUT)
    ...
    switch(g_u8_display_mode[u8Dev])
    {
        ...
        case MS1826_SEAMLESS_MATRIX_SWITCHER_OUT:
            for(u8_tx_id = 0 ; u8_tx_id < 4 ; u8_tx_id ++)
            {
                if(Vipss_Mapping_Rx[u8Dev][u8_tx_id] == u8_path)
                {
                    if(b_freeze_status[u8Dev][u8_tx_id]==FALSE)    //add by wf8421 20230523
                    {
                        single_video_mute(u8Dev,u8_tx_id, b_mute);
                    }
                }
            }
            break;
        case MS1826_VIDEO_SPLITTER:
            ...
            {
//                if(Vipss_Mapping_Rx[u8Dev][u8_tx_id] == u8_path)
                if(g_u8SplitterDispMapping[u8Dev][u8_tx_id] == u8_path)
                {
                    splitter_video_mute(u8Dev,u8_tx_id, b_mute);
                }
            }
            //splitter_video_mute(u8Dev,u8_path, b_mute);
            break;
...
//	mculib_i2c_write_16bidx8bval(0xb6,0x34,0x0f);	//关闭4路timgen
//	mculib_i2c_write_16bidx8bval(0xb6,0x34,0xff);	//打开4路timgen
    mculib_i2c_write_16bidx8bval(gu8ChipAddr[u8Dev],0x34,0x0f);   //关闭4路timgen
    mculib_i2c_write_16bidx8bval(gu8ChipAddr[u8Dev],0x34,0xff);   //打开4路timgen
    ...
    _video_mute(u8Dev,HDMI_RX_A, MS_TRUE);
    _video_mute(u8Dev,HDMI_RX_B, MS_TRUE);
    _video_mute(u8Dev,HDMI_RX_C, MS_TRUE);
    _video_mute(u8Dev,HDMI_RX_D, MS_TRUE);
    app_osd_init(u8Dev);
...
VOID video_process_cfg(UINT8 u8Dev)
{
    ...
    _video_timgen_cfg(u8Dev,TIMGEN_4);
	mculib_i2c_write_16bidx8bval(gu8ChipAddr[u8Dev],0x34,0x0f);	//关闭4路timgen
	mculib_i2c_write_16bidx8bval(gu8ChipAddr[u8Dev],0x34,0xff);	//打开4路timg
    app_osd_init(u8Dev);
    g_bOSDShown[u8Dev] = MS_TRUE;
    sys_show_osd(u8Dev,g_bOSDShown[u8Dev], 0x0F);
    _video_port_hdmi_tx_init(u8Dev);
...
VOID _video_misc_process(UINT8 u8Dev,UINT8 u8_rx_id)
{
    ...
    switch(g_stVideo_HDMIRx_Timing[u8Dev][u8_rx_id].u8_polarity)
    {
        ...
        case ProgrVNegHPos:
        case InterVNegHPos:
            g_st_rx_polarity[u8Dev][u8_rx_id].u8_hsync_flip_en  = MS_FALSE;
            g_st_rx_polarity[u8Dev][u8_rx_id].u8_vsync_flip_en  = MS_TRUE;
            break;
        case ProgrVPosHNeg:
        case InterVPosHNeg:
            g_st_rx_polarity[u8Dev][u8_rx_id].u8_hsync_flip_en  = MS_TRUE;
            g_st_rx_polarity[u8Dev][u8_rx_id].u8_vsync_flip_en  = MS_FALSE;
            break;
        case ProgrVPosHPos:
        case InterVPosHPos:
            g_st_rx_polarity[u8Dev][u8_rx_id].u8_hsync_flip_en  = MS_FALSE;
    ...
    for(u8_vipss_id = 0; u8_vipss_id < 4; u8_vipss_id ++)
    {
        ...
        bSetVipssCscFlag = MS_FALSE;
        switch(g_u8_display_mode[u8Dev])
        {
            case MS1826_SEAMLESS_MATRIX_SWITCHER:
            case MS1826_SEAMLESS_MATRIX_SWITCHER_OUT:
                {
                    if(Vipss_Mapping_Rx[u8Dev][u8_vipss_id] == u8_vipss_mapping)
        ...
        //if(Vipss_Mapping_Rx[u8Dev][u8_vipss_id] == u8_vipss_mapping)
        //if(Vipss_Mapping_Rx[u8_vipss_id] == g_u8_HDMI_RX_PROT[u8_rx_id])
        if(bSetVipssCscFlag)
        {
            ...
            if(g_st_hdmi_rx[u8Dev][u8_rx_id].b_is_hdmi == MS_FALSE)
            ...
            ms1826_hal_vehc_trig(u8_vipss_id+VEHC_VIPSS_1,MS_TRUE);     // 解决上电和插拔输入端，亮度对比度恢复默认的问题
        }
    }
}
...
static VOID _video_tx_process(UINT8 u8Dev)
{
    ...
    for(u8_tx_id = 0; u8_tx_id < 4; u8_tx_id ++)
    {
        b_hpd = api_hdmi_tx_get_hpd_status(&g_st_hdmi_tx[u8Dev][u8_tx_id]);
        if(b_hpd != b_hpd_bak[u8Dev][u8_tx_id])
        {
            b_hpd_bak[u8Dev][u8_tx_id]  = b_hpd;
            if(b_hpd)
            {
                switch(u8_tx_id)
                {
                    case 0:
                        api_hdmi_tx_get_edid(&g_st_hdmi_tx[u8Dev][u8_tx_id],g_st_hdmi_tx[u8Dev][u8_tx_id].pu8_edid_buf,&g_hdmi_edid_flag[u8Dev][u8_tx_id],FALSE);
            ...
            else
            {
                ...
                {
                    case 0:
                        memset(g_st_hdmi_tx[u8Dev][u8_tx_id].pu8_edid_buf,0,256);
...
static VOID _video_rx_process(UINT8 u8Dev,UINT8 u8_rx_idx)
{
    UINT8 u8_tx_id = 0;

//    LOG("[INFO] hdmi%d b_audio_pll_manu_cfg = %d\r\n", u8_rx_idx,g_st_hdmi_rx[u8Dev][u8_rx_idx].b_audio_pll_manu_cfg);

    api_hdmi_rx_task(&g_st_hdmi_rx[u8Dev][u8_rx_idx]);

    if(api_hdmi_rx_is_audio_changed(&g_st_hdmi_rx[u8Dev][u8_rx_idx]))
    {
        LOG("[INFO] hdmi%d audio change\r\n", u8_rx_idx);
#if 0
        for(u8_tx_id = 0; u8_tx_id < 4; u8_tx_id ++)
        {
            if(u8_Tx_audio_sel[u8Dev][u8_tx_id] == u8_rx_idx)
            {
                if(g_st_hdmi_tx[u8Dev][u8_tx_id].u8_audio_src == FROM_HDMI_RX_AUDIO)
                {
                    LOG("[INFO] hdmi%d tx audio change\r\n", u8_tx_id);
                    api_hdmi_tx_audio_mute(&g_st_hdmi_tx[u8Dev][u8_tx_id], MS_TRUE);
                    memcpy((&g_st_hdmi_tx[u8Dev][u8_tx_id].st_infoframes_packets.st_audio_infoframe), &g_st_hdmi_rx[u8Dev][u8_rx_idx].st_packets_infoframes.st_audio_infoframe, sizeof(HDMI_AUDIO_INFOFRAME_T));
                    api_hdmi_tx_audio_change(&g_st_hdmi_tx[u8Dev][u8_tx_id]);
//                    if(g_bFirstStartupFlag[u8Dev][u8_tx_id] == FALSE)
                        api_hdmi_tx_audio_mute(&g_st_hdmi_tx[u8Dev][u8_tx_id], b_audio_mute[u8Dev][u8_tx_id]);
                }
            }
        }
#endif
    ...
    if(api_hdmi_rx_is_video_changed(&g_st_hdmi_rx[u8Dev][u8_rx_idx]))
    {
        LOG("[INFO] hdmi%d video change\r\n", u8_rx_idx);
        _video_mute(u8Dev,u8_rx_idx, TRUE);
        if(api_hdmi_rx_get_video_timing(&g_st_hdmi_rx[u8Dev][u8_rx_idx]) == MS_ERR_OK)
        {
            LOG("[INFO] hdmi%d video timing ok\r\n", u8_rx_idx);
            g_st_hdmi_rx[u8Dev][u8_rx_idx].b_timing_stable = TRUE;

            if(_timing_compare(&g_stVideo_HDMIRx_Timing[u8Dev][u8_rx_idx],  &(g_st_hdmi_rx[u8Dev][u8_rx_idx].st_rx_video_timing)) == FALSE)
            {
                LOG("[INFO] hdmi%d video timing is different\r\n", u8_rx_idx);
                _video_mute(u8Dev,u8_rx_idx, TRUE);
                memcpy(&g_stVideo_HDMIRx_Timing[u8Dev][u8_rx_idx],&(g_st_hdmi_rx[u8Dev][u8_rx_idx].st_rx_video_timing),sizeof(HDMI_VIDEO_TIMING_T));
                if(g_stVideo_HDMIRx_Timing[u8Dev][u8_rx_idx].u8_polarity % 2 == 0)
                {
                    g_stVideo_HDMIRx_Timing[u8Dev][u8_rx_idx].u16_vactive /= 2;
                }
                LOG("u8_polarity = %d\r\n",g_stVideo_HDMIRx_Timing[u8Dev][u8_rx_idx].u8_polarity);
                LOG("u16_htotal  = %d\r\n",g_stVideo_HDMIRx_Timing[u8Dev][u8_rx_idx].u16_htotal);
                LOG("u16_vtotal  = %d\r\n",g_stVideo_HDMIRx_Timing[u8Dev][u8_rx_idx].u16_vtotal);
                LOG("u16_hactive = %d\r\n",g_stVideo_HDMIRx_Timing[u8Dev][u8_rx_idx].u16_hactive);
                LOG("u16_vactive = %d\r\n",g_stVideo_HDMIRx_Timing[u8Dev][u8_rx_idx].u16_vactive);
                LOG("u16_pixclk  = %d\r\n",g_stVideo_HDMIRx_Timing[u8Dev][u8_rx_idx].u16_pixclk);
                LOG("u16_vfreq   = %d\r\n",g_stVideo_HDMIRx_Timing[u8Dev][u8_rx_idx].u16_vfreq);
                LOG("u16_hoffset = %d\r\n",g_stVideo_HDMIRx_Timing[u8Dev][u8_rx_idx].u16_hoffset);
                LOG("u16_voffset = %d\r\n",g_stVideo_HDMIRx_Timing[u8Dev][u8_rx_idx].u16_voffset);
                LOG("u16_hsyncwidth = %d\r\n",g_stVideo_HDMIRx_Timing[u8Dev][u8_rx_idx].u16_hsyncwidth);
                LOG("u16_vsyncwidth = %d\r\n",g_stVideo_HDMIRx_Timing[u8Dev][u8_rx_idx].u16_vsyncwidth);
                LOG("hdmi format %s\r\n", g_st_hdmi_rx[u8Dev][u8_rx_idx].b_is_hdmi?"HDMI":"DVI");
                b_RxVideoChg[u8Dev][u8_rx_idx] = TRUE;
            }
            else
            {
                LOG("[INFO] hdmi%d video timing same b_RxVideoChg = %d\r\n", u8_rx_idx,b_RxVideoChg[u8Dev][u8_rx_idx]);
                if(b_RxVideoChg[u8Dev][u8_rx_idx] == MS_FALSE)
                {
                    //_video_mute(u8Dev,u8_rx_idx, FALSE);
                    b_RxVideoChg[u8Dev][u8_rx_idx] = TRUE;
                }
            }
        }
        ...
        {
            memset(&g_stVideo_HDMIRx_Timing[u8Dev][u8_rx_idx],NULL,sizeof(HDMI_VIDEO_TIMING_T));
            g_st_hdmi_rx[u8Dev][u8_rx_idx].b_timing_stable = FALSE;
//            b_RxVideoChg[u8Dev][u8_rx_idx] = TRUE;
//            _video_mute(u8Dev,u8_rx_idx, TRUE);
        }
        g_bOSDShown[u8Dev] = MS_TRUE;
        sys_show_osd(u8Dev,g_bOSDShown[u8Dev], 1<<u8_rx_idx);
        _video_misc_process(u8Dev,u8_rx_idx);
    }
    else
    {
        if(g_st_hdmi_rx[u8Dev][u8_rx_idx].u8_rx_state == RX_STATE_RUNNING && b_RxVideoChg[u8Dev][u8_rx_idx])
        {
            LOG("Video change rx[%d]\r\n",u8_rx_idx);
            b_RxVideoChg[u8Dev][u8_rx_idx] = FALSE;
            switch(g_u8_display_mode[u8Dev])
            {
                case MS1826_SEAMLESS_MATRIX_SWITCHER:
                case MS1826_SEAMLESS_MATRIX_SWITCHER_OUT:
                    for(u8_tx_id = 0 ; u8_tx_id < 4 ; u8_tx_id ++)
                    {
                        if(Vipss_Mapping_Rx[u8Dev][u8_tx_id] == u8_rx_idx)
                        {
                            app_single_process(u8Dev,u8_tx_id);
                        }
                    }
					#if 1
					if(g_st_hdmi_rx[u8Dev][u8_rx_idx].b_timing_stable == MS_FALSE)
					{
						LOG("[INFO] hdmi%d video no stable to mute video\r\n",u8_rx_idx);
						app_MS1826SetVideoMute(u8Dev,u8_rx_idx,TRUE);
					}
					#endif
                    break;
                case MS1826_VIDEO_SPLITTER:
                    ...
                    {
                        //if(Vipss_Mapping_Rx[u8Dev][u8_tx_id] == u8_rx_idx)
                        if(g_u8SplitterDispMapping[u8Dev][u8_tx_id] == u8_rx_idx)
                        {
                            app_splitter_process(u8Dev,u8_tx_id);
                        }
                    }
                #else
                    app_splitter_process(u8Dev,u8_rx_idx);
                #endif
                    break;
                case MS1826_VIDEO_SPLICER:
                    if(gst_splicer_app_dev[u8Dev].u8_src_sel == u8_rx_idx)
                    {
                        app_splicer_process(u8Dev,&gst_splicer_app_dev[u8Dev]);
                    }
                    break;
            }
            mculib_delay_ms(200);
            video_bypass_tx_cfg(u8Dev,u8_rx_idx);
            switch (u8_rx_idx)
            {
                case 0:
                    LOG("[INFO] hdmi1 cur status: %d\r\n", g_st_hdmi_rx[u8Dev][u8_rx_idx].u8_rx_state);
                    break;
                case 1:
                    LOG("[INFO] hdmi2 cur status: %d\r\n", g_st_hdmi_rx[u8Dev][u8_rx_idx].u8_rx_state);
                    break;
                case 2:
                    LOG("[INFO] hdmi3 cur status: %d\r\n", g_st_hdmi_rx[u8Dev][u8_rx_idx].u8_rx_state);
                    break;
                case 3:
                    LOG("[INFO] hdmi4 cur status: %d\r\n", g_st_hdmi_rx[u8Dev][u8_rx_idx].u8_rx_state);
                    break;
                default:
                    break;
            }
    ...
    for(u8_tx_id = 0 ; u8_tx_id < 4 ; u8_tx_id ++)
    {
        ...
        if(Vipss_Mapping_Rx[u8Dev][u8_tx_id] == u8_rx_idx)
        {
            if(g_u16Startup2InitTimer[u8Dev][u8_tx_id]==0)
            {
                if(g_bFirstStartupFlag[u8Dev][u8_tx_id])
                {
                    g_bFirstStartupFlag[u8Dev][u8_tx_id] = FALSE;
                    app_ms1826_audio_input_switch(u8Dev,g_stMs1826Event[u8Dev].u8TxAudioSourceSta[u8_tx_id],u8_tx_id);
                    api_hdmi_tx_audio_mute(&g_st_hdmi_tx[u8Dev][u8_tx_id], b_audio_mute[u8Dev][u8_tx_id]);
                }
            }
        }
...
#if ENABLE_MS1826_FRAME_LOCK
    if(g_bPreTimingStable[u8Dev][u8_rx_idx] != g_st_hdmi_rx[u8Dev][u8_rx_idx].b_timing_stable)
    {
        g_bPreTimingStable[u8Dev][u8_rx_idx] = g_st_hdmi_rx[u8Dev][u8_rx_idx].b_timing_stable;

        for(u8_tx_id = 0 ; u8_tx_id < 4 ; u8_tx_id ++)
        {
            if(g_u8FrameLockRxTable[u8Dev][u8_tx_id] == u8_rx_idx)
            {
                if(g_st_hdmi_rx[u8Dev][u8_rx_idx].b_timing_stable)
                {
                    _video_timgen_cfg_frame_lock(u8Dev,u8_rx_idx,TIMGEN_1+u8_tx_id);
                }
                else
                {
                    _video_timgen_cfg(u8Dev,TIMGEN_1+u8_tx_id);
                }
            }
        }
    }
    ...
    if(g_st_hdmi_rx[u8Dev][u8_rx_idx].u8_rx_state != RX_STATE_RUNNING)
        _video_mute(u8Dev,u8_rx_idx, TRUE);
}
...
static VOID _video_rx_process_out(UINT8 u8Dev,UINT8 u8_rx_idx)
{
    UINT8 u8_tx_id = 0;
...
//    api_hdmi_rx_manual_eq(&g_st_hdmi_rx[u8Dev][u8_rx_idx],1,1,1,MS_TRUE);

    u8_rx_clock = ms1826_hal_hdmi_rx_get_tmds_clk(u8_rx_idx);
//  LOG("[INFO] hdmi u8_rx_clock %d \r\n", u8_rx_clock);
    if(u8_rx_clock == 0)
    {
        _video_mute(u8Dev,u8_rx_idx, MS_TRUE);
		u32_mute_cnt[u8_rx_idx] = 0;
        g_st_hdmi_rx[u8Dev][u8_rx_idx].b_timing_stable = FALSE;
    }
    else
    {
		//if(u8_rx_change_bak[u8Dev][u8_rx_idx] == 1)
		{
			if(u32_mute_cnt[u8_rx_idx] > 10)
			{
//                _video_mute(u8Dev,u8_rx_idx, MS_FALSE);

#if 1
                if(!g_st_hdmi_rx[u8Dev][u8_rx_idx].b_timing_stable)
                {
					mculib_i2c_write_16bidx8bval(gu8ChipAddr[u8Dev],0x4004 + u8_rx_idx * 0x100, 0x10);    // freerun lock
					mculib_i2c_write_16bidx8bval(gu8ChipAddr[u8Dev],0x4004 + u8_rx_idx * 0x100, 0x11);    // once lock
#if ENABLE_MS1826_OUT_MODE_RELOAD_EDID
                    LOG("[INFO] hdmi %d video change reload edid\r\n", u8_rx_idx);
                    api_hdmi_rx_edid_update(&g_st_hdmi_rx[u8Dev][u8_rx_idx],(UINT8 *)g_u8c_sys_edid_default_buf,256);
#endif
                }
				  _video_mute(u8Dev,u8_rx_idx, MS_FALSE);
#endif
...
//    				_video_mute(u8Dev,u8_rx_idx, MS_FALSE);
                    api_hdmi_tx_audio_mute(&g_st_hdmi_tx[u8Dev][u8_tx_id], b_audio_mute[u8Dev][u8_tx_id]);
//                    app_digital_audio_cfg(u8Dev,u8_Tx_audio_sel[u8Dev][u8_rx_idx], g_u8_i2s_idx[u8_tx_id], b_digital_audio_mute[u8Dev][u8_tx_id]?FALSE:TRUE);
//                    app_digital_audio_cfg(u8Dev,u8_Tx_audio_sel[u8Dev][u8_rx_idx], g_u8_spdif_idx[u8_tx_id], b_digital_audio_mute[u8Dev][u8_tx_id]?FALSE:TRUE);
                }
            }
        }
    }
    ...
    if(g_bPreTimingStable[u8Dev][u8_rx_idx] != g_st_hdmi_rx[u8Dev][u8_rx_idx].b_timing_stable)
    {
        ...
        if(g_st_hdmi_rx[u8Dev][u8_rx_idx].b_timing_stable)
        {
		    LOG("[INFO] hdmi %d video change b_afif_underflow = %d\r\n", u8_rx_idx,g_st_hdmi_rx[u8Dev][u8_rx_idx].st_rx_pdec_sts.b_afif_underflow);
		    LOG("[INFO] hdmi %d video change b_afif_overflow  = %d\r\n", u8_rx_idx,g_st_hdmi_rx[u8Dev][u8_rx_idx].st_rx_pdec_sts.b_afif_overflow);
			ms1826_hal_hdmi_rx_audio_hbr_pll_cfg(u8_rx_idx, g_st_hdmi_rx[u8Dev][u8_rx_idx].st_rx_pdec_sts.b_hbr_rcv);
			ms1826_hal_hdmi_rx_audio_fifo_reset(u8_rx_idx, MS_TRUE);
			mculib_delay_ms(1);
			ms1826_hal_hdmi_rx_audio_fifo_reset(u8_rx_idx, MS_FALSE);
			mculib_delay_ms(1);
			ms1826_hal_hdmi_rx_hbr_en(u8_rx_idx, g_st_hdmi_rx[u8Dev][u8_rx_idx].st_rx_pdec_sts.b_hbr_rcv);
        }
#endif
    }
#endif
    ...
    if(u8_rx_change_bak[u8Dev][u8_rx_idx] == 1)
    {
        ...
        api_hdmi_rx_manual_eq(&g_st_hdmi_rx[u8Dev][u8_rx_idx],MS1826_RX_EQ_VALUE_CH1,MS1826_RX_EQ_VALUE_CH2,MS1826_RX_EQ_VALUE_CH3,MS_TRUE);
        #endif
		u32_rx_mdt_ints = ms1826_hal_hdmi_rx_get_mdt_ints_ext(u8_rx_idx);
		api_hdmi_rx_update_state(&g_st_hdmi_rx[u8Dev][u8_rx_idx]);
		if(u32_rx_mdt_ints & 0x3045)
			ms1826_hal_hdmi_rx_phy_process(u8_rx_idx, g_st_hdmi_rx[u8Dev][u8_rx_idx].st_rx_ints.b_clk_change, u8_rx_clock);
//		ms1826_hal_hdmi_rx_phy_process(u8_rx_idx, g_st_hdmi_rx[u8Dev][u8_rx_idx].st_rx_ints.b_clk_change, u8_rx_clock);

		if(g_st_hdmi_rx[u8Dev][u8_rx_idx].st_rx_pdec_sts.b_afif_underflow || g_st_hdmi_rx[u8Dev][u8_rx_idx].st_rx_pdec_sts.b_afif_overflow)
		{
//		    LOG("[INFO] hdmi %d b_afif_underflow = %d\r\n", u8_rx_idx,g_st_hdmi_rx[u8Dev][u8_rx_idx].st_rx_pdec_sts.b_afif_underflow);
//		    LOG("[INFO] hdmi %d b_afif_overflow  = %d\r\n", u8_rx_idx,g_st_hdmi_rx[u8Dev][u8_rx_idx].st_rx_pdec_sts.b_afif_overflow);
			ms1826_hal_hdmi_rx_audio_hbr_pll_cfg(u8_rx_idx, g_st_hdmi_rx[u8Dev][u8_rx_idx].st_rx_pdec_sts.b_hbr_rcv);
			ms1826_hal_hdmi_rx_audio_fifo_reset(u8_rx_idx, MS_TRUE);
			mculib_delay_ms(1);
			ms1826_hal_hdmi_rx_audio_fifo_reset(u8_rx_idx, MS_FALSE);
		...
		api_hdmi_rx_config(&g_st_hdmi_rx[u8Dev][u8_rx_idx]);
        #if MS1826_RX_MANUAL_EQ_ENABLE
        api_hdmi_rx_manual_eq(&g_st_hdmi_rx[u8Dev][u8_rx_idx],MS1826_RX_EQ_VALUE_CH1,MS1826_RX_EQ_VALUE_CH2,MS1826_RX_EQ_VALUE_CH3,MS_TRUE);
        #endif
		if(api_hdmi_rx_is_audio_changed(&g_st_hdmi_rx[u8Dev][u8_rx_idx]))
		{
			...
			for(u8_tx_id = 0; u8_tx_id < 4; u8_tx_id ++)
			{
				if(u8_Tx_audio_sel[u8Dev][u8_tx_id] == u8_rx_idx)
				{
                    ...
                    {
    					api_hdmi_tx_audio_mute(&g_st_hdmi_tx[u8Dev][u8_tx_id], MS_TRUE);
    					if(g_bFirstStartupFlag[u8Dev][u8_tx_id] == TRUE)
    					{
                            if(g_bDigitalAudioSwByHdmiTx[u8Dev][u8_tx_id])
                                g_stMs1826Event[u8Dev].u8TxDigitalAudioSource[u8_tx_id] = u8_Tx_audio_sel[u8Dev][u8_tx_id];
                            app_digital_audio_cfg(u8Dev,g_stMs1826Event[u8Dev].u8TxDigitalAudioSource[u8_tx_id], g_u8_i2s_idx[u8_tx_id], FALSE);
    ...
    if(api_hdmi_rx_is_video_changed(&g_st_hdmi_rx[u8Dev][u8_rx_idx]))
    {
        ...
        {
            LOG("[INFO] hdmi%d video timing ok\r\n", u8_rx_idx);
            //g_st_hdmi_rx[u8_rx_idx].b_timing_stable = TRUE;
            if(_timing_compare(&g_stVideo_HDMIRx_Timing[u8Dev][u8_rx_idx],  &(g_st_hdmi_rx[u8Dev][u8_rx_idx].st_rx_video_timing)) == FALSE)
            {
...
//            g_st_hdmi_rx[u8Dev][u8_rx_idx].b_timing_stable = TRUE;
    ...
    else
    {
        ...
        {
            LOG("Video change rx[%d]\r\n",u8_rx_idx);
            u8_rx_change_bak[u8Dev][u8_rx_idx] = 1;
            b_RxVideoChg[u8Dev][u8_rx_idx] = FALSE;
            switch(g_u8_display_mode[u8Dev])
            {
                case MS1826_SEAMLESS_MATRIX_SWITCHER:
...
//			mculib_delay_ms(50);
//            _video_mute(u8Dev,u8_rx_idx, FALSE);
            LOG("[INFO] hdmi %d video  mute off \r\n", u8_rx_idx);
        }
    }
}
#endif
...
static VOID _video_tx_hdcp_process(UINT8 u8Dev)
{
    ...
    for(u8_tx_id = 0; u8_tx_id < 4; u8_tx_id ++)
    {
        if(b_hdcp_status && u8_tx_id == 0)
        {
            if(api_hdmi_tx_hdcp14_get_status(&g_st_hdmi_tx[u8Dev][u8_tx_id]))
            {
...
//              LOG("[INFO] HDCP failed...\r\n");
        ...
        b_hpd = api_hdmi_tx_get_hpd_status(&g_st_hdmi_tx[u8Dev][u8_tx_id]);
        if(b_hpd != b_hpd_bakes[u8Dev][u8_tx_id])
        {
            b_hpd_bakes[u8Dev][u8_tx_id]    = b_hpd;
            if(b_hpd)
            {
                switch(u8_tx_id)
                {
...
//                      api_hdmi_tx_hdcp14_init(&g_st_hdmi_tx[u8Dev][u8_tx_id]);
//                      mculib_delay_ms(200);
//                      api_hdmi_tx_hdcp14_enable(&g_st_hdmi_tx[u8Dev][u8_tx_id], MS_TRUE);
//                      b_hdcp_status = TRUE;
                        LOG("[INFO] HDMI Tx1 connected....\r\n");
                        break;
                    case 1:
...
//  static UINT8 u8_rx_idx = 0;
    ...
    for(u8_rx_idx = 0; u8_rx_idx < 4; u8_rx_idx++)
    {
        if(g_u8_display_mode[u8Dev]==MS1826_SEAMLESS_MATRIX_SWITCHER_OUT)
            _video_rx_process_out(u8Dev,u8_rx_idx);
        else
            _video_rx_process(u8Dev,u8_rx_idx);
    }
}
...
VOID osd_service(UINT8 u8Dev)
{
    ...
    if (!g_bOSDShown[u8Dev] && !b_osd_menu)
    {
    ...
    if (g_u8OSDCount[u8Dev] > u8_osd_time) // count every 500ms
    {
        g_u8OSDCount[u8Dev] = 0;
        sys_show_osd(u8Dev, FALSE, 0x0F);
        g_bOSDShown[u8Dev] = FALSE;
    }
}
