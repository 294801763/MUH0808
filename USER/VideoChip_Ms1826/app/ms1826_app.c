...
UINT8 const g_u8_i2s_idxdev2[4] 			= {DIG_AUDIO_I2S4, DIG_AUDIO_I2S3, DIG_AUDIO_I2S2, DIG_AUDIO_I2S1};//配置模拟音频口
...
const stTimingTable_t res_ms1826table[]=
{
    ...
    { 1920,   2200,   148,   88,   44,    Hpos ,   1080,  1125,  36,   4,    5,  Vpos,     30,    34,                 7425,    PROG},//1920x1080@30Hz
	{ 1920,   2640,   148,   88,   44,    Hpos ,   1080,  1125,  36,   4,    5,  Vpos,     25,    33,                 7425,    PROG},//1920x1080@25Hz
...
HDMI_VIDEO_TIMING_T	g_stVideo_HDMIRx_Timing[DEV_NUM_MS1826][4];
MISC_POLARITY_T	g_st_rx_polarity[DEV_NUM_MS1826][4];
CSC_DEV_T g_st_vipss_csc[DEV_NUM_MS1826][4];
BOOL b_RxVideoChg[DEV_NUM_MS1826][4] = {{MS_FALSE}};
UINT8 u8_wdma_buf_id[DEV_NUM_MS1826] = {0};
UINT8 u8_rdma_buf_id[DEV_NUM_MS1826] = {0};
...
BOOL g_bOSDShown[DEV_NUM_MS1826] = {MS_FALSE};
UINT8 g_u8OSDCount[DEV_NUM_MS1826];
BOOL g_user_osd_str_enable[DEV_NUM_MS1826][4] = {{MS_FALSE}};
...
BOOL g_bUserOsdStringIsAlignRight[DEV_NUM_MS1826][4] = {{MS_FALSE}};
BOOL g_bUserOsdStringDisplayEnable[DEV_NUM_MS1826][4] = {{MS_FALSE}};

SPLIT_DEV_T g_st_split_var[DEV_NUM_MS1826][4];

UINT8 g_u8_display_mode[DEV_NUM_MS1826]	= {MS1826_SEAMLESS_MATRIX_SWITCHER};//MS1826_VIDEO_SPLITTER;
UINT8 g_u8_display_mode_bak[DEV_NUM_MS1826] = {MS1826_VIDEO_SPLICER};
UINT8 g_u8_output_index[DEV_NUM_MS1826][4] = {{1, 1, 1, 1}};
UINT8 Matrix_Tx_Mapping[DEV_NUM_MS1826][4] = {{0, 1, 2, 3}};
UINT8 Vipss_Mapping_Rx[DEV_NUM_MS1826][4] =	{{0, 1, 2, 3}};
...
#elif (CHIP_INTERNAL_PROCESS_COLORSPACE == HDMI_COLORSPACE_YCBCR444)
VIDEO_MIXER_COLOR_T g_st_bg_color = {0x00, 0x80, 0x80};
#else
...
stMs1826FunctionEvent_t g_stMs1826Event[DEV_NUM_MS1826];
...
extern UINT32 ms1826_hal_hdmi_rx_get_mdt_ints_ext(UINT8 u8_rx_idx);
extern VOID ms1826_hal_hdmi_rx_phy_power_toggle(UINT8 u8_rx_idx);
extern UINT8 ms1826_hal_hdmi_rx_phy_process(UINT8 u8_rx_idx, BOOL b_rx_tmds_clk_change, UINT16 u16_rx_tmds_clk);
extern UINT16 ms1826_hal_hdmi_rx_get_tmds_clk(UINT8 u8_rx_idx);
extern VOID _single_mode_frc_cfg(UINT8 u8Dev);

void app_ms1826_variable_init(UINT8 u8Dev,UINT8 u8WorkMode)
{
    UINT8 i;

    u8_wdma_buf_id[u8Dev]   = 0;
    u8_rdma_buf_id[u8Dev]   = 0;

    g_bOSDShown[u8Dev]      = MS_FALSE;
    g_u8OSDCount[u8Dev]     = 0;

    g_u8_display_mode[u8Dev] = u8WorkMode;//;
    g_u8_display_mode_bak[u8Dev] = MS1826_VIDEO_SPLICER;
    ...
    for(i=0;i<4;i++)
    {
        ...
        b_RxVideoChg[u8Dev][i]          = MS_FALSE;
        b_hpd_bak[u8Dev][i]             = MS_FALSE;
        g_u8_output_index[u8Dev][i]     = 1;
        Matrix_Tx_Mapping[u8Dev][i]     = i;
        Bypass_Tx_Status[u8Dev][i]      = 0;
        Vipss_Mapping_Rx[u8Dev][i]      = i;
        Bypass_Tx_Status[u8Dev][i]      = 0;
        u8_Tx_audio_sel[u8Dev][i]       = 0;
        g_u8TxFormat[u8Dev][i]          = MS1826_TX_HDMI;
        b_audio_mute[u8Dev][i]          = MS_FALSE;
        b_digital_audio_mute[u8Dev][i]  = MS_FALSE;
        g_user_osd_str_enable[u8Dev][i] = MS_FALSE;
        ...
        g_bUserOsdStringIsAlignRight[u8Dev][i]  = MS_FALSE;
        g_bUserOsdStringDisplayEnable[u8Dev][i] = MS_FALSE;
        g_bSplicerEnable[u8Dev][i]      = MS_FALSE;
        g_bSplitterEnable[u8Dev][i]     = MS_FALSE;
    #if ENABLE_MS1826_FRAME_LOCK
        g_u8FrameLockRxTable[u8Dev][i] = i;
    #endif
        g_bPreTimingStable[u8Dev][i] = MS_FALSE;
        g_bDigitalAudioSwByHdmiTx[u8Dev][i] = MS_TRUE;
        g_u8SplitterDispMapping[u8Dev][i] = i;

        g_stMs1826Event[u8Dev].bPortChangeFlag[i] = MS_FALSE;
        g_stMs1826Event[u8Dev].u8OutputSel[i] = i;

        g_stMs1826Event[u8Dev].bFreezeChangeFlag[i] = MS_FALSE;
        g_stMs1826Event[u8Dev].u8FreezeSta[i] = 0;

        g_stMs1826Event[u8Dev].bTxResolutionChangeFlag[i] = MS_FALSE;
        g_stMs1826Event[u8Dev].u8TxResolutionID[i] = 1;

        g_stMs1826Event[u8Dev].bBrightnessAdjustFlag[i] = MS_FALSE;
        g_stMs1826Event[u8Dev].u8Brightness[i] = 128;
        g_stMs1826Event[u8Dev].bContrastAdjustFlag[i] = MS_FALSE;
        g_stMs1826Event[u8Dev].u8Contrast[i] = 128;
        g_stMs1826Event[u8Dev].bSaturationAdjustFlag[i] = MS_FALSE;
        g_stMs1826Event[u8Dev].u8Saturation[i] = 128;
        g_stMs1826Event[u8Dev].bHueAdjustFlag[i] = MS_FALSE;
        g_stMs1826Event[u8Dev].u8Hue[i] = 0;

        g_stMs1826Event[u8Dev].bRxEdidChangeFlag[i] = MS_FALSE;

        g_stMs1826Event[u8Dev].bFreezeSwitchFlag[i] = MS_FALSE;

        g_stMs1826Event[u8Dev].bTxCfgChangeFlag[i] = MS_FALSE;
        g_stMs1826Event[u8Dev].u8TxFormat[i] = 0;
        g_stMs1826Event[u8Dev].u8TxColorSpace[i] = 0;

        g_stMs1826Event[u8Dev].bTxHdcp14ChangeFlag[i] = MS_FALSE;
        g_stMs1826Event[u8Dev].u8TxHdcp14Sta[i] = 0;
        g_stMs1826Event[u8Dev].bReadTxEdidFlag[i] = MS_FALSE;

        g_stMs1826Event[u8Dev].bTxDigitalAudioChangeFlag[i] = MS_FALSE;
        g_stMs1826Event[u8Dev].u8TxDigitalAudioSta[i] = 0;
        g_stMs1826Event[u8Dev].u8TxDigitalAudioSource[i] = i;

        g_stMs1826Event[u8Dev].bTxAudioSourceChangeFlag[i] = MS_FALSE;
        g_stMs1826Event[u8Dev].u8TxAudioSourceSta[i] = i;

        g_stMs1826Event[u8Dev].bTxPatternChangeFlag[i] = MS_FALSE;
        g_stMs1826Event[u8Dev].u8TxPatternID[i] = 0;
        g_stMs1826Event[u8Dev].u8TxPatternEnable[i] = 0;

        g_stMs1826Event[u8Dev].bColorChangeFlag[i] = MS_FALSE;
        g_stMs1826Event[u8Dev].u8ColorEnable[i] = 0;
        g_stMs1826Event[u8Dev].u8RedValue[i] = 128;
        g_stMs1826Event[u8Dev].u8GreenValue[i] = 128;
        g_stMs1826Event[u8Dev].u8BlueValue[i] = 128;

        g_stMs1826Event[u8Dev].bTxTimingChangeFlag[i] = MS_FALSE;
        g_stMs1826Event[u8Dev].bSplitChangeFlag[i] = MS_FALSE;
        g_stMs1826Event[u8Dev].u8SplitEnable[i] = 0;

        g_stMs1826Event[u8Dev].bBypassPortChangeFlag[i] = MS_FALSE;
        g_stMs1826Event[u8Dev].u8BypassOutputSel[i] = i;

        g_stMs1826Event[u8Dev].bOsdStringChangeFlag[i] = MS_FALSE;
        memset(g_stMs1826Event[u8Dev].u8OsdStr[i],' ',MS1826_OSD_MAX_LEN);

        g_stMs1826Event[u8Dev].bVideoMuteChangeFlag[i] = MS_FALSE;
        g_stMs1826Event[u8Dev].bVideoMuteSta[i] = MS_FALSE;

        g_stMs1826Event[u8Dev].bTxHdmiAudioMuteChangeFlag[i] = MS_FALSE;
        g_stMs1826Event[u8Dev].u8TxHdmiAudioMuteSta[i] = 0;

        g_stMs1826Event[u8Dev].bSplicerChangeFlag[i] = MS_FALSE;
        g_stMs1826Event[u8Dev].stSplicerVar[i].u16X = 0;
        g_stMs1826Event[u8Dev].stSplicerVar[i].u16Y = 0;
        g_stMs1826Event[u8Dev].stSplicerVar[i].u16W = 0;
        g_stMs1826Event[u8Dev].stSplicerVar[i].u16H = 0;
        g_stMs1826Event[u8Dev].bSplicerEnable[i] = MS_FALSE;
    }
    g_stMs1826Event[u8Dev].bWorkModeSwitch = MS_FALSE;
    g_stMs1826Event[u8Dev].u8WorkMode = MS_FALSE;
    g_stMs1826Event[u8Dev].u8SubMode = 0;

    g_stMs1826Event[u8Dev].u16FreezeDelayTime = 0;
    g_stMs1826Event[u8Dev].bFreezeTimeEnd = MS_FALSE;
...
VOID app_digital_audio_cfg(UINT8 u8Dev,UINT8 u8_audio_src, UINT8 u8_digital_audio_id, BOOL b_status)
{
	...
	g_st_misc_digital_audio_cfg.u8_audio_src = g_u8_HDMI_RX_PROT[Vipss_Mapping_Rx[u8Dev][u8_audio_src]];
...
//	g_st_misc_mclk_cfg.b_mclk_from_crystal = MS_FALSE;
	...
	g_st_misc_mclk_cfg.u8_audio_src = g_u8_HDMI_RX_PROT[Vipss_Mapping_Rx[u8Dev][u8_audio_src]];
    g_st_misc_mclk_cfg.b_mclk_from_crystal= MS_FALSE;
...
VOID app_tx_audio_cfg(UINT8 u8Dev,UINT8	u8_audio_src, UINT8 u8_tx)
{
    ...
    if(g_st_hdmi_tx[u8Dev][u8_tx].u8_audio_src == FROM_HDMI_RX_AUDIO)
    {
    	...
    	st_misc_tx_audio_sel.u8_audio_src = g_u8_HDMI_RX_PROT[Vipss_Mapping_Rx[u8Dev][u8_tx]];
...
VOID app_display_mode_cfg(UINT8 u8Dev,UINT8 u8Mode,UINT8 u8SubMode)
{
	UINT8 u8_i;

    g_bOSDShown[u8Dev] = MS_FALSE;
    sys_show_osd(u8Dev,g_bOSDShown[u8Dev], 0x0F);
    g_u8_display_mode_bak[u8Dev] = g_u8_display_mode[u8Dev];
    g_u8_display_mode[u8Dev] = u8Mode;
    api_hdmi_tx_output_en(&g_st_hdmi_tx[u8Dev][HDMI_TX_A], MS_FALSE);
    api_hdmi_tx_output_en(&g_st_hdmi_tx[u8Dev][HDMI_TX_B], MS_FALSE);
    api_hdmi_tx_output_en(&g_st_hdmi_tx[u8Dev][HDMI_TX_C], MS_FALSE);
    api_hdmi_tx_output_en(&g_st_hdmi_tx[u8Dev][HDMI_TX_D], MS_FALSE);
    video_ms1826_reset(u8Dev,g_u8_display_mode[u8Dev], g_u8_display_mode_bak[u8Dev]);
    switch (g_u8_display_mode[u8Dev])
    {
        case MS1826_VIDEO_SPLITTER:
            g_st_app_splitter_dev[u8Dev].b_splitter_sw = MS_FALSE;
        ...
        case MS1826_VIDEO_SPLICER:
            gst_splicer_app_dev[u8Dev].b_splicer_sw = MS_FALSE;
            gst_splicer_app_dev[u8Dev].u8_tx_mirror = MS_FALSE;
...
//            app_single_mode_config(u8Dev);
			app_single_mode_rx_switch(u8Dev,Vipss_Mapping_Rx[u8Dev][0], 0);
			app_single_mode_rx_switch(u8Dev,Vipss_Mapping_Rx[u8Dev][1], 1);
			app_single_mode_rx_switch(u8Dev,Vipss_Mapping_Rx[u8Dev][2], 2);
			app_single_mode_rx_switch(u8Dev,Vipss_Mapping_Rx[u8Dev][3], 3);
			LOG("[INFO] MS1826 function is SEAMLESS_MATRIX_SWITCHER.");
            break;
        case MS1826_BYPASS_MATRIX_SWITCHER:
            for(u8_i = 0; u8_i < 4; u8_i ++)
...
VOID app_digital_input_hdmi_output_cfg(UINT8 u8Dev,UINT8 g_u8_digital_idx,UINT8 u8_tx)
{
	...
	g_st_misc_digital_audio_cfg.u8_audio_src = RX1_AUDIO+Vipss_Mapping_Rx[u8Dev][u8_tx];
	...
	api_hdmi_tx_audio_mute(&g_st_hdmi_tx[u8Dev][u8_tx], MS_TRUE);
	api_misc_tx_audio_in_cfg(&st_misc_tx_audio_sel);
	api_hdmi_tx_audio_mute(&g_st_hdmi_tx[u8Dev][u8_tx], MS_FALSE);

// modify by wf8421 20230718 to disable mclk
	g_st_misc_mclk_cfg.u8_mclk_div = MISC_CLKDIV_1;
	g_st_misc_mclk_cfg.u8_digital_audio_dst = u8_digital_audio_dst;
	g_st_misc_mclk_cfg.u8_mclk_en = MS_FALSE;
	g_st_misc_mclk_cfg.u8_audio_src = g_u8_HDMI_RX_PROT[Vipss_Mapping_Rx[u8Dev][u8_tx]];
	g_st_misc_mclk_cfg.b_mclk_from_crystal = MS_FALSE;
	api_misc_digital_mclk_cfg(&g_st_misc_mclk_cfg);
}
...
//		api_vehc_instance_init(&g_st_vovehc[u8_vehc_idx], u8_vehc_idx);
		if(CHIP_INTERNAL_PROCESS_COLORSPACE == HDMI_COLORSPACE_RGB)
		{
			...
			g_st_vovehc[u8Dev][u8_vehc_idx].b_yuv = MS_FALSE;
...
//		api_vehc_instance_init(&g_st_vivehc[u8Dev][u8_vehc_idx], u8_vehc_idx);
		if(CHIP_INTERNAL_PROCESS_COLORSPACE == HDMI_COLORSPACE_RGB)
		{
			...
			g_st_vivehc[u8Dev][u8_vehc_idx].b_yuv = MS_FALSE;
...
//                ||u8_dis_bak == MS1826_SEAMLESS_MATRIX_SWITCHER_OUT)
            {
...
void app_ms1826_output_resolution(UINT8 u8Dev,UINT8 u8TxID,UINT8 u8ResID)
{
	...
	api_hdmi_tx_output_en(&g_st_hdmi_tx[u8Dev][u8TxID], MS_FALSE);
...
//	api_misc_timing_get(g_arrOutputTable[u8ResID], &g_stVideo_HDMITx_Timing[u8TxID]);
	...
	st_pll_dev.u8_pll_idx = PLLV1 + u8TxID;
    ...
    switch(g_stVideo_HDMITx_Timing[u8Dev][u8TxID].u8_polarity)
    {
        ...
        case ProgrVNegHPos:
        case InterVNegHPos:
            l_stTxPolarity.u8_hsync_flip_en = MS_FALSE;
            l_stTxPolarity.u8_vsync_flip_en = MS_TRUE;
            break;
        case ProgrVPosHNeg:
        case InterVPosHNeg:
            l_stTxPolarity.u8_hsync_flip_en = MS_TRUE;
            l_stTxPolarity.u8_vsync_flip_en = MS_FALSE;
            break;
        case ProgrVPosHPos:
        case InterVPosHPos:
            l_stTxPolarity.u8_hsync_flip_en = MS_FALSE;
    ...
    if((g_u8_display_mode[u8Dev] == MS1826_SEAMLESS_MATRIX_SWITCHER)
        ||(g_u8_display_mode[u8Dev] == MS1826_SEAMLESS_MATRIX_SWITCHER_OUT))
    {
	    app_single_process(u8Dev,u8TxID);
//	    app_single_mode_config(u8Dev);
        if(g_u8_display_mode[u8Dev] == MS1826_SEAMLESS_MATRIX_SWITCHER_OUT)
        {
...
//	g_st_hdmi_tx[u8Dev][u8TxID].u8_audio_src = FROM_HDMI_RX_AUDIO;
	...
	g_bOSDShown[u8Dev] = MS_TRUE;
	sys_show_osd(u8Dev,g_bOSDShown[u8Dev], 1<<u8TxID);
}
...
{
	#if (_PROJECT_CURRENT_SELECT == _PROJECT_LMX_4OUHS_C)   //
	app_digital_audio_cfg(u8Dev,u8_audio_src, g_u8_i2s_idx[u8_tx], b_digital_audio_mute[u8Dev][u8_tx]?FALSE:TRUE);
	//app_digital_audio_cfg(u8Dev,u8_audio_src, g_u8_spdif_idx[u8_tx], b_digital_audio_mute[u8Dev][u8_tx]?FALSE:TRUE);  //为了解决输出的模拟音频关闭任意其它一路都会导致第一路关闭
...
void app_ms1826_pattern_config(UINT8 u8Dev,UINT8 u8_vehc_idx,UINT8 u8_pattern_id,BOOL b_enable)
{
    ...
    //  g_st_hdmi_tx[u8Dev][u8_vehc_idx].u8_audio_src = FROM_HDMI_RX_AUDIO;
        ...
        api_hdmi_tx_output_en(&g_st_hdmi_tx[u8Dev][u8_vehc_idx], MS_FALSE);
...
{
    pstRxTiming->u16HActive = g_stVideo_HDMIRx_Timing[u8Dev][u8_rx].u16_hactive;
    pstRxTiming->u16HTotal  = g_stVideo_HDMIRx_Timing[u8Dev][u8_rx].u16_htotal;
    pstRxTiming->u16HBP     = g_stVideo_HDMIRx_Timing[u8Dev][u8_rx].u16_hoffset-g_stVideo_HDMIRx_Timing[u8Dev][u8_rx].u16_hsyncwidth;
    pstRxTiming->u16HFP     = g_stVideo_HDMIRx_Timing[u8Dev][u8_rx].u16_vactive;
    pstRxTiming->u16HSync   = g_stVideo_HDMIRx_Timing[u8Dev][u8_rx].u16_hsyncwidth;

    pstRxTiming->u16VActive = g_stVideo_HDMIRx_Timing[u8Dev][u8_rx].u16_vactive;
    pstRxTiming->u16VTotal  = g_stVideo_HDMIRx_Timing[u8Dev][u8_rx].u16_vtotal;
    pstRxTiming->u16VBP     = g_stVideo_HDMIRx_Timing[u8Dev][u8_rx].u16_voffset-g_stVideo_HDMIRx_Timing[u8Dev][u8_rx].u16_vsyncwidth;
    pstRxTiming->u16VFP     = g_stVideo_HDMIRx_Timing[u8Dev][u8_rx].u16_vactive;
    pstRxTiming->u16VSync   = g_stVideo_HDMIRx_Timing[u8Dev][u8_rx].u16_vsyncwidth;

    pstRxTiming->u8VClk     = (g_stVideo_HDMIRx_Timing[u8Dev][u8_rx].u16_vfreq+10)/100;
    pstRxTiming->u16PClk    = g_stVideo_HDMIRx_Timing[u8Dev][u8_rx].u16_pixclk;
    pstRxTiming->u8Interlace= g_stVideo_HDMIRx_Timing[u8Dev][u8_rx].u8_polarity%2?PROG:INTR;

    switch(g_stVideo_HDMIRx_Timing[u8Dev][u8_rx].u8_polarity)
    {
        case ProgrVNegHNeg:
        case InterVNegHNeg:
...
{
#if 1
    if(g_u8_display_mode[u8Dev] == MS1826_VIDEO_SPLITTER)
    {
        ...
        app_splitter_user_mode_win_config(u8Dev,u8WinID,&l_stMS1826WinCfg,MS_FALSE,u8Update?MS_TRUE:MS_FALSE,TRUE);
    }
#endif
...
void app_ms1826_tx_split_update(u8 u8Dev,u8 u8Path)
{
    if(g_u8_display_mode[u8Dev] == MS1826_VIDEO_SPLITTER)
    {
        app_splitter_user_mode_win_update(u8Dev,MS_FALSE,u8Path);
    }
}

void app_ms1826_get_osd_string(u8 u8Dev,u8 u8Rx)
{
    memcpy(p_str,g_stMs1826Event[u8Dev].u8OsdStr[u8Rx],MS1826_OSD_MAX_LEN);
}
...
//    {
//        g_bOSDShown[u8Dev] = MS_TRUE;
//        sys_show_osd(u8Dev,g_bOSDShown[u8Dev], (1<<u8Rx));
//        g_u8_time = 3;
//    }
...
void app_ms1826_user_string_display(u8 u8Dev)
{
    ...
    for(i=0;i<4;i++)
    {
        if(g_bUserOsdStringDisplayEnable[u8Dev][i])
        {
            ...
            {
                g_bOSDShown[u8Dev] = MS_TRUE;
                sys_show_osd(u8Dev,g_bOSDShown[u8Dev], (1<<i));
                g_u8_time = 3;
            }
            else
            {
...
void app_ms1826_tx_splicer_config(u8 u8Dev,u8 u8PortIndex,u16 u16X,u16 u16Y,u16 u16W,u16 u16H)
{
    ...
    stMs1826WinTable.u8Rx = Vipss_Mapping_Rx[u8Dev][u8PortIndex];
...
void app_ms1826_set_splitter_mode(u8 u8Dev,u8 u8Mode)
{
    ...
    if(u8Mode==0)
    {
        if(g_u8_display_mode[u8Dev]!=cg_stMs1826DevCfgTable[u8Dev].u8WorkMode)
        {
            //app_display_mode_cfg(u8Dev,cg_stMs1826DevCfgTable[u8Dev].u8WorkMode,0);
    ...
    else
    {
        ...
        {
            //app_display_mode_cfg(u8Dev,MS1826_VIDEO_SPLITTER,u8Mode);
            app_mode_switch_cfg(u8Dev,MS1826_VIDEO_SPLITTER,u8Mode);
        }
    }
#else
...
{
    g_stMs1826Event[u8Dev].u8OutputSel[u8Tx] = u8Rx;
    g_stMs1826Event[u8Dev].bPortChangeFlag[u8Tx] = TRUE;
}

void app_MS1826BypassSwitch(u8 u8Dev,u8 u8Rx, u8 u8Tx)
{
    g_stMs1826Event[u8Dev].u8BypassOutputSel[u8Tx] = u8Rx;
    g_stMs1826Event[u8Dev].bBypassPortChangeFlag[u8Tx] = TRUE;
}

void app_MS1826FreezeChange(u8 u8Dev,u8 u8Tx, u8 u8Sta)
{
    g_stMs1826Event[u8Dev].bFreezeChangeFlag[u8Tx] = TRUE;
    g_stMs1826Event[u8Dev].u8FreezeSta[u8Tx] = u8Sta;
}

void app_MS1826TxResolutionChange(u8 u8Dev,u8 u8Tx, u8 u8ResID)
{
    g_stMs1826Event[u8Dev].bTxResolutionChangeFlag[u8Tx] = TRUE;
    g_stMs1826Event[u8Dev].u8TxResolutionID[u8Tx] = u8ResID;
}

void app_MS1826SwitchFreeze(u8 u8Dev,u8 u8Tx, u16 u16Time)
{
    g_stMs1826Event[u8Dev].bFreezeSwitchFlag[u8Tx] = TRUE;
    g_stMs1826Event[u8Dev].u16FreezeDelayTime = u16Time;  //time unit 1ms
    g_stMs1826Event[u8Dev].bFreezeTimeEnd = FALSE;
}

void app_MS1826SwFreezeTimer(u8 u8Dev)
{
    if(g_stMs1826Event[u8Dev].u16FreezeDelayTime)
    {
        g_stMs1826Event[u8Dev].u16FreezeDelayTime--;
        if(g_stMs1826Event[u8Dev].u16FreezeDelayTime==0)
        {
            g_stMs1826Event[u8Dev].bFreezeTimeEnd = TRUE;
        }
    }
}
...
void app_MS1826PictureAdjust(u8 u8Dev,u8 u8Tx,u8 u8Value,u8 u8AdjMode)
{
    ...
    {
        g_stMs1826Event[u8Dev].bBrightnessAdjustFlag[u8Tx] = TRUE;
        g_stMs1826Event[u8Dev].u8Brightness[u8Tx] = u8Value;
    }
    if((u8AdjMode&MS1826_ADJUST_CONTRAST)==MS1826_ADJUST_CONTRAST)
    {
        g_stMs1826Event[u8Dev].bContrastAdjustFlag[u8Tx] = TRUE;
        g_stMs1826Event[u8Dev].u8Contrast[u8Tx] = u8Value;
    }
    if((u8AdjMode&MS1826_ADJUST_SATURATION)==MS1826_ADJUST_SATURATION)
    {
        g_stMs1826Event[u8Dev].bSaturationAdjustFlag[u8Tx] = TRUE;
        g_stMs1826Event[u8Dev].u8Saturation[u8Tx] = u8Value;
    }
    if((u8AdjMode&MS1826_ADJUST_HUE)==MS1826_ADJUST_HUE)
    {
        g_stMs1826Event[u8Dev].bHueAdjustFlag[u8Tx] = TRUE;
        g_stMs1826Event[u8Dev].u8Hue[u8Tx] = u8Value;
    }
}

void app_MS1826TxConfig(u8 u8Dev,u8 u8Tx,u8 u8Value)
{
    g_stMs1826Event[u8Dev].bTxCfgChangeFlag[u8Tx] = TRUE;
    g_stMs1826Event[u8Dev].u8TxFormat[u8Tx] = u8Value;
    g_stMs1826Event[u8Dev].u8TxColorSpace[u8Tx] = CHIP_TX_CSC_OUT_COLORSPACE;
}

void app_MS1826SetTxColorSpace(u8 u8Dev,u8 u8Tx,u8 u8Value)
{
    g_stMs1826Event[u8Dev].bTxCfgChangeFlag[u8Tx] = TRUE;
    g_stMs1826Event[u8Dev].u8TxColorSpace[u8Tx] = u8Value;
}

void app_MS1826TxHdcpEnable(u8 u8Dev,u8 u8Tx,u8 u8Value)
{
    g_stMs1826Event[u8Dev].bTxHdcp14ChangeFlag[u8Tx] = TRUE;
    g_stMs1826Event[u8Dev].u8TxHdcp14Sta[u8Tx] = u8Value;
}

void app_MS1826RxEdidUpdate(u8 u8Dev,u8 u8Tx,pu8 pu8Edidbuf,u16 u16Size)
{
    g_stMs1826Event[u8Dev].bRxEdidChangeFlag[u8Tx] = TRUE;
    memcpy(g_stMs1826Event[u8Dev].u8EdidBuf[u8Tx],pu8Edidbuf,u16Size);
}

void app_MS1826GetTxEdid(u8 u8Dev,u8 u8Tx)
{
    g_stMs1826Event[u8Dev].bReadTxEdidFlag[u8Tx] = TRUE;
}

void app_MS1826TxDigitalAudioMute(u8 u8Dev,u8 u8Tx,u8 u8Value)
{
    g_stMs1826Event[u8Dev].bTxDigitalAudioChangeFlag[u8Tx] = TRUE;
    g_stMs1826Event[u8Dev].u8TxDigitalAudioSta[u8Tx] = u8Value;
}

void app_MS1826TxDigitalAudioSourceSelect(u8 u8Dev,u8 u8Tx,u8 u8AudioSource)
{
    g_stMs1826Event[u8Dev].bTxDigitalAudioChangeFlag[u8Tx] = TRUE;
    g_stMs1826Event[u8Dev].u8TxDigitalAudioSource[u8Tx] = u8AudioSource;
    g_bDigitalAudioSwByHdmiTx[u8Dev][u8Tx] = FALSE;
}

void app_MS1826TxAudioSourceSwitch(u8 u8Dev,u8 u8Tx,u8 u8Value)
{
    g_stMs1826Event[u8Dev].bTxAudioSourceChangeFlag[u8Tx] = TRUE;
    g_stMs1826Event[u8Dev].u8TxAudioSourceSta[u8Tx] = u8Value;
}

void app_MS1826SetUserTiming(u8 u8Dev,u8 u8Tx,pstTimingTable_t pstTiming)
{
    g_stMs1826Event[u8Dev].bTxTimingChangeFlag[u8Tx] = TRUE;
    memcpy(&g_stMs1826Event[u8Dev].stTiming[u8Tx],pstTiming,sizeof(stTimingTable_t));
}

void app_MS1826TxPatternConfig(u8 u8Dev,u8 u8Tx,u8 u8PatternID,u8 u8En)
{
    g_stMs1826Event[u8Dev].bTxPatternChangeFlag[u8Tx] = TRUE;
    g_stMs1826Event[u8Dev].u8TxPatternID[u8Tx] = u8PatternID;
    g_stMs1826Event[u8Dev].u8TxPatternEnable[u8Tx] = u8En;
}

void app_MS1826PictureColorConfig(u8 u8Dev,u8 u8Tx,u8 u8RVal,u8 u8GVal,u8 u8BVal,u8 u8En)
{
    g_stMs1826Event[u8Dev].bColorChangeFlag[u8Tx] = TRUE;
    g_stMs1826Event[u8Dev].u8RedValue[u8Tx]        = u8RVal;
    g_stMs1826Event[u8Dev].u8GreenValue[u8Tx]      = u8GVal;
    g_stMs1826Event[u8Dev].u8BlueValue[u8Tx]       = u8BVal;
    g_stMs1826Event[u8Dev].u8ColorEnable[u8Tx]     = u8En;
}

void app_MS1826SplitConfig(u8 u8Dev,u8 u8PortID,u8 u8Enable,u16 u16X,u16 u16Y,u16 u16W,u16 u16H)
{
    g_stMs1826Event[u8Dev].bSplitChangeFlag[u8PortID] = TRUE;
    g_stMs1826Event[u8Dev].u8SplitEnable[u8PortID]=u8Enable;
    g_stMs1826Event[u8Dev].stSplitPosision[u8PortID].u16_horizontal_start=u16X;
    g_stMs1826Event[u8Dev].stSplitPosision[u8PortID].u16_horizontal_active=u16W;
    g_stMs1826Event[u8Dev].stSplitPosision[u8PortID].u16_vertical_start=u16Y;
    g_stMs1826Event[u8Dev].stSplitPosision[u8PortID].u16_vertical_active=u16H;
}

void app_MS1826WorkModeSwitch(u8 u8Dev,u8 u8WorkMode,u8 u8SubMode)
{
    g_stMs1826Event[u8Dev].bWorkModeSwitch = TRUE;
    g_stMs1826Event[u8Dev].u8WorkMode = u8WorkMode;
    g_stMs1826Event[u8Dev].u8SubMode = u8SubMode;
}

void app_MS1826SetOsdString(u8 u8Dev,u8 u8Rx,u8 u8Len,u8 *pu8Str,bool bIsAlignRight)
{
    g_stMs1826Event[u8Dev].bOsdStringChangeFlag[u8Rx] = TRUE;
    g_stMs1826Event[u8Dev].bOsdStringIsAlignRight[u8Rx] = bIsAlignRight;
    g_bUserOsdStringIsAlignRight[u8Dev][u8Rx] = bIsAlignRight;
    memset(g_stMs1826Event[u8Dev].u8OsdStr[u8Rx],' ',MS1826_OSD_MAX_LEN);
    if(g_bUserOsdStringIsAlignRight[u8Dev][u8Rx])
    {
        memcpy(g_stMs1826Event[u8Dev].u8OsdStr[u8Rx]+(MS1826_OSD_MAX_LEN-u8Len),pu8Str,u8Len);
    }
    else
        memcpy(g_stMs1826Event[u8Dev].u8OsdStr[u8Rx],pu8Str,u8Len);
}

void app_MS1826SetUserOsdStringEnable(u8 u8Dev,u8 u8Rx, bool bEnable)
{
    g_stMs1826Event[u8Dev].bOsdStringDisplayChangeFlag[u8Rx]    = TRUE;;
    g_stMs1826Event[u8Dev].bOsdStringDisplayEnable[u8Rx]        = bEnable;
}

void app_MS1826SetUserStringColor(u8 u8Dev,u8 u8Rx, u8 u8R,u8 u8G,u8 u8B)
{
    g_stMs1826Event[u8Dev].bOsdStringColorChangeFlag[u8Rx]  = TRUE;
    g_stMs1826Event[u8Dev].bOsdStringColorRed[u8Rx]         = u8R;
    g_stMs1826Event[u8Dev].bOsdStringColorGreen[u8Rx]       = u8G;
    g_stMs1826Event[u8Dev].bOsdStringColorBlue[u8Rx]        = u8B;
}

void app_MS1826SetUserOsdStringPosition(u8 u8Dev,u8 u8Rx, u16 u16PosX,u16 u16PosY)
{
    g_stMs1826Event[u8Dev].bOsdStringPositionChangeFlag[u8Rx]   = TRUE;
    g_stMs1826Event[u8Dev].u16OsdStringPositionX[u8Rx]          = u16PosX;
    g_stMs1826Event[u8Dev].u16OsdStringPositionY[u8Rx]          = u16PosY;
}

void app_MS1826SetVideoMute(u8 u8Dev,u8 u8Rx,bool bSta)
{
    g_stMs1826Event[u8Dev].bVideoMuteChangeFlag[u8Rx] = TRUE;
    g_stMs1826Event[u8Dev].bVideoMuteSta[u8Rx] = bSta;
}

void app_MS1826TxHdmiAudioMute(u8 u8Dev,u8 u8Tx,u8 u8Value)
{
    g_stMs1826Event[u8Dev].bTxHdmiAudioMuteChangeFlag[u8Tx] = TRUE;
    g_stMs1826Event[u8Dev].u8TxHdmiAudioMuteSta[u8Tx] = u8Value;
}

void app_MS1826SplicerConfig(u8 u8Dev,u8 u8PortID,u16 u16X,u16 u16Y,u16 u16W,u16 u16H)
{
    g_stMs1826Event[u8Dev].bSplicerChangeFlag[u8PortID] = TRUE;
    g_stMs1826Event[u8Dev].stSplicerVar[u8PortID].u16X=u16X;
    g_stMs1826Event[u8Dev].stSplicerVar[u8PortID].u16W=u16W;
    g_stMs1826Event[u8Dev].stSplicerVar[u8PortID].u16Y=u16Y;
    g_stMs1826Event[u8Dev].stSplicerVar[u8PortID].u16H=u16H;
}

void app_MS1826SplicerEnable(u8 u8Dev,u8 u8PortID,bool bEn)
{
    g_stMs1826Event[u8Dev].bSplicerChangeFlag[u8PortID] = TRUE;
    g_stMs1826Event[u8Dev].bSplicerEnable[u8PortID]=bEn;
}

#if ENABLE_MS1826_FRAME_LOCK
void app_MS1826FrameLockSelect(u8 u8Dev,u8 u8RxIndex,u8 u8TxIndex)
{
    g_stMs1826Event[u8Dev].bTxFrameLockChangeFlag[u8TxIndex] = TRUE;
    g_stMs1826Event[u8Dev].u8TxFrameLockRxSelect[u8TxIndex]=u8RxIndex;
}
#endif

void app_MS1826SetSplitterMode(u8 u8Dev,u8 u8Mode)
{
    g_stMs1826Event[u8Dev].bSplitterModeChangeFlag = TRUE;
    g_stMs1826Event[u8Dev].u8SplitterMode = u8Mode;
}

void app_MS1826SetSplitterUpdate(u8 u8Dev,u8 u8Path)
{
    g_stMs1826Event[u8Dev].bSplitUpdateChangeFlag = TRUE;
    g_stMs1826Event[u8Dev].u8SplitPath = u8Path;
}
void app_MS1826SetHpd(u8 u8Dev,u8 RX,u8 en)
{
    g_stMs1826Event[u8Dev].bHpdChangeFlag[RX] = TRUE;
    g_stMs1826Event[u8Dev].bHpdEn = en;
}
void app_MS1826FunctionProcess(u8 u8Dev)
{
    ...
    for(i=0;i<4;i++)
    {
		if(g_stMs1826Event[u8Dev].bHpdChangeFlag[i])
        {
            if(g_stMs1826Event[u8Dev].bHpdEn)
            {
                for(j=0;j<4;j++)
                {
				LOG("TX[%d] api_hdmi_rx_set_hpd,FALSE\r\n",i);
                 api_hdmi_rx_set_hpd(&g_st_hdmi_rx[u8Dev][j],FALSE);
					 mculib_delay_ms(500);
                }
            }
            else
            {
               for(j=0;j<4;j++)
                {
				LOG("TX[%d] api_hdmi_rx_set_hpd,TRUE\r\n",i);
                 api_hdmi_rx_set_hpd(&g_st_hdmi_rx[u8Dev][j],TRUE);
                }
            }
            g_stMs1826Event[u8Dev].bHpdChangeFlag[i]  = FALSE;
        }
        if(g_stMs1826Event[u8Dev].bPortChangeFlag[i])   //视频切换需要先单独配置完成，否则声音通道对应错误
        {
            switch(g_u8_display_mode[u8Dev])
            {
                case MS1826_SEAMLESS_MATRIX_SWITCHER:
                case MS1826_SEAMLESS_MATRIX_SWITCHER_OUT:
                    {
                        if(g_u8_display_mode[u8Dev] == MS1826_SEAMLESS_MATRIX_SWITCHER_OUT)
                        {
                            app_single_process(u8Dev,i);
                        }
                        app_single_mode_rx_switch(u8Dev,g_stMs1826Event[u8Dev].u8OutputSel[i], i);
                        g_u8SplitterDispMapping[u8Dev][i] = g_stMs1826Event[u8Dev].u8OutputSel[i];
                    }
                    break;
                case MS1826_VIDEO_SPLITTER:
                    {
                        app_splitter_mode_rx_switch(u8Dev,g_stMs1826Event[u8Dev].u8OutputSel[i], i);
                    }
                    break;
            }
            LOG("TX[%d] source switch to %d\r\n",i,g_stMs1826Event[u8Dev].u8OutputSel[i]);
            g_stMs1826Event[u8Dev].bPortChangeFlag[i] = FALSE;
        }

        if(g_stMs1826Event[u8Dev].bSplicerChangeFlag[i])
        {
            g_stMs1826Event[u8Dev].bSplicerChangeFlag[i] = FALSE;
            g_bSplicerEnable[u8Dev][i] = g_stMs1826Event[u8Dev].bSplicerEnable[i];
            app_ms1826_tx_splicer_config(u8Dev,i,
                        g_stMs1826Event[u8Dev].stSplicerVar[i].u16X,
                        g_stMs1826Event[u8Dev].stSplicerVar[i].u16Y,
                        g_stMs1826Event[u8Dev].stSplicerVar[i].u16W,
                        g_stMs1826Event[u8Dev].stSplicerVar[i].u16H);
...
#if ENABLE_MS1826_FRAME_LOCK
        if(g_stMs1826Event[u8Dev].bTxFrameLockChangeFlag[i])
        {
            g_stMs1826Event[u8Dev].bTxFrameLockChangeFlag[i] = FALSE;
            app_ms1826_frame_lock_select(u8Dev,g_stMs1826Event[u8Dev].u8TxFrameLockRxSelect[i],i);
            LOG("TX[0x%02x] set frame lock to %d\r\n",i,g_stMs1826Event[u8Dev].u8TxFrameLockRxSelect[i]);
        }
#endif
    }
    ...
    for(i=0;i<4;i++)
    {
        if(g_stMs1826Event[u8Dev].bBypassPortChangeFlag[i])
        {
            Matrix_Tx_Mapping[u8Dev][i] = g_stMs1826Event[u8Dev].u8BypassOutputSel[i];
            switch_bypass_channel(u8Dev,g_stMs1826Event[u8Dev].u8BypassOutputSel[i], i);
            LOG("TX[%d] source bypass switch to %d\r\n",i,g_stMs1826Event[u8Dev].u8BypassOutputSel[i]);
            g_stMs1826Event[u8Dev].bBypassPortChangeFlag[i] = FALSE;
        }
        if(g_stMs1826Event[u8Dev].bFreezeChangeFlag[i])
        {
            g_stMs1826Event[u8Dev].bFreezeChangeFlag[i] = FALSE;
            if(g_stMs1826Event[u8Dev].u8FreezeSta[i])
            {
                if((g_u8_display_mode[u8Dev] == MS1826_SEAMLESS_MATRIX_SWITCHER_OUT)
                    ||(g_u8_display_mode[u8Dev] == MS1826_SEAMLESS_MATRIX_SWITCHER))
                    _single_video_freeze(u8Dev,i);
                else if(g_u8_display_mode[u8Dev] == MS1826_VIDEO_SPLITTER)
                    _splitter_video_freeze(u8Dev,i);
                else if(g_u8_display_mode[u8Dev] == MS1826_VIDEO_SPLICER)
        ...
        if(g_stMs1826Event[u8Dev].bTxCfgChangeFlag[i])
        {
            g_stMs1826Event[u8Dev].bTxCfgChangeFlag[i] = FALSE;
            app_ms1826_tx_format_config(u8Dev,i,g_stMs1826Event[u8Dev].u8TxFormat[i]?TRUE:FALSE,g_stMs1826Event[u8Dev].u8TxColorSpace[i]);
            LOG("TX[%d] format: %d\r\n",i,g_stMs1826Event[u8Dev].u8TxFormat[i]);
        }
        if(g_stMs1826Event[u8Dev].bTxHdcp14ChangeFlag[i])
        {
            g_stMs1826Event[u8Dev].bTxHdcp14ChangeFlag[i] = FALSE;
            app_ms1826_tx_hdcp14_enable(u8Dev,i,g_stMs1826Event[u8Dev].u8TxHdcp14Sta[i]?TRUE:FALSE);
            LOG("TX[%d] HDCP status: %d\r\n",i,g_stMs1826Event[u8Dev].u8TxHdcp14Sta[i]);
        }
        if(g_stMs1826Event[u8Dev].bReadTxEdidFlag[i])
        {
            u8 u8Edid[256];
            u16 j;
            g_stMs1826Event[u8Dev].bReadTxEdidFlag[i] = FALSE;
        ...
        if(g_stMs1826Event[u8Dev].bTxTimingChangeFlag[i])
        {
            g_stMs1826Event[u8Dev].bTxTimingChangeFlag[i] = FALSE;
            app_ms1826_tx_timing_config(u8Dev,i,&g_stMs1826Event[u8Dev].stTiming[i]);
        }

        if(g_stMs1826Event[u8Dev].bFreezeSwitchFlag[i])
        {
            g_stMs1826Event[u8Dev].bFreezeSwitchFlag[i] = FALSE;
            sl_u8FreezeFlag |= 1<<i;
            l_u8StartFreezeFlag = 1;
        }

        if(g_stMs1826Event[u8Dev].bTxDigitalAudioChangeFlag[i])
        {
            g_stMs1826Event[u8Dev].bTxDigitalAudioChangeFlag[i] = FALSE;
//            if(g_bDigitalAudioSwByHdmiTx[u8Dev][i])
//                g_stMs1826Event[u8Dev].u8TxDigitalAudioSource[i] = u8_Tx_audio_sel[u8Dev][i];
            app_ms1826_audio_output_mute(u8Dev,g_stMs1826Event[u8Dev].u8TxDigitalAudioSource[i],i,g_stMs1826Event[u8Dev].u8TxDigitalAudioSta[i]?TRUE:FALSE);
            LOG("TX[%d] digital audio source: %d\r\n",i,g_stMs1826Event[u8Dev].u8TxDigitalAudioSource[i]);
            LOG("TX[%d] digital audio mute: %s\r\n",i,g_stMs1826Event[u8Dev].u8TxDigitalAudioSta[i]?"TRUE":"FALSE");
        }
        if(g_stMs1826Event[u8Dev].bTxPatternChangeFlag[i])
        {
            g_stMs1826Event[u8Dev].bTxPatternChangeFlag[i] = FALSE;
            app_ms1826_pattern_config(u8Dev,i,g_stMs1826Event[u8Dev].u8TxPatternID[i],g_stMs1826Event[u8Dev].u8TxPatternEnable[i]?TRUE:FALSE);
//            app_ms18226_rx_pattern_config(u8Dev,i,g_stMs1826Event[u8Dev].u8TxPatternID[i],g_stMs1826Event[u8Dev].u8TxPatternEnable[i]?TRUE:FALSE);
        }

        if(g_stMs1826Event[u8Dev].bColorChangeFlag[i])
        {
            g_stMs1826Event[u8Dev].bColorChangeFlag[i] = FALSE;
            app_ms1826_colortemp_adjust(u8Dev,g_stMs1826Event[u8Dev].u8ColorEnable[i]?TRUE:FALSE,
                                        i,
                                        g_stMs1826Event[u8Dev].u8RedValue[i],
                                        g_stMs1826Event[u8Dev].u8GreenValue[i],
                                        g_stMs1826Event[u8Dev].u8BlueValue[i]);
        }

        if(g_stMs1826Event[u8Dev].bSplitChangeFlag[i])
        {
            g_stMs1826Event[u8Dev].bSplitChangeFlag[i] = FALSE;

            app_ms1826_tx_split_config(u8Dev,i,
                                g_stMs1826Event[u8Dev].u8SplitEnable[i],
                                g_stMs1826Event[u8Dev].stSplitPosision[i].u16_horizontal_start,
                                g_stMs1826Event[u8Dev].stSplitPosision[i].u16_vertical_start,
                                g_stMs1826Event[u8Dev].stSplitPosision[i].u16_horizontal_active,
                                g_stMs1826Event[u8Dev].stSplitPosision[i].u16_vertical_active);
            LOG("TX[%d] split config: en[%d] x[%d] y[%d] w[%d] h[%d]\r\n",i,g_stMs1826Event[u8Dev].u8SplitEnable[i],
                                g_stMs1826Event[u8Dev].stSplitPosision[i].u16_horizontal_start,
                                g_stMs1826Event[u8Dev].stSplitPosision[i].u16_vertical_start,
                                g_stMs1826Event[u8Dev].stSplitPosision[i].u16_horizontal_active,
                                g_stMs1826Event[u8Dev].stSplitPosision[i].u16_vertical_active);
        ...
        if(g_stMs1826Event[u8Dev].bBrightnessAdjustFlag[i])
        {
            g_stMs1826Event[u8Dev].bBrightnessAdjustFlag[i] = FALSE;
//            app_ms1826_picture_adjust(u8Dev,i,g_stMs1826Event[u8Dev].u8Brightness[i],MS1826_ADJUST_BRIGHTNESS);
            app_ms1826_picture_adjust(u8Dev,i+VEHC_VIPSS_1,g_stMs1826Event[u8Dev].u8Brightness[i],MS1826_ADJUST_BRIGHTNESS);
            LOG("TX[%d] u8Brightness: %d\r\n",i,g_stMs1826Event[u8Dev].u8Brightness[i]);
        }
        if(g_stMs1826Event[u8Dev].bContrastAdjustFlag[i])
        {
            g_stMs1826Event[u8Dev].bContrastAdjustFlag[i] = FALSE;
//            app_ms1826_picture_adjust(u8Dev,i,g_stMs1826Event[u8Dev].u8Contrast[i],MS1826_ADJUST_CONTRAST);
            app_ms1826_picture_adjust(u8Dev,i+VEHC_VIPSS_1,g_stMs1826Event[u8Dev].u8Contrast[i],MS1826_ADJUST_CONTRAST);
            LOG("TX[%d]   u8Contrast: %d\r\n",i,g_stMs1826Event[u8Dev].u8Contrast[i]);
        }
        if(g_stMs1826Event[u8Dev].bSaturationAdjustFlag[i])
        {
            g_stMs1826Event[u8Dev].bSaturationAdjustFlag[i] = FALSE;
//            app_ms1826_picture_adjust(u8Dev,i,g_stMs1826Event[u8Dev].u8Saturation[i],MS1826_ADJUST_SATURATION);
            app_ms1826_picture_adjust(u8Dev,i+VEHC_VIPSS_1,g_stMs1826Event[u8Dev].u8Saturation[i],MS1826_ADJUST_SATURATION);
            LOG("TX[%d] u8Saturation: %d\r\n",i,g_stMs1826Event[u8Dev].u8Saturation[i]);
        }
        if(g_stMs1826Event[u8Dev].bHueAdjustFlag[i])
        {
            g_stMs1826Event[u8Dev].bHueAdjustFlag[i] = FALSE;
//            app_ms1826_picture_adjust(u8Dev,i,g_stMs1826Event[u8Dev].u8Hue[i],MS1826_ADJUST_HUE);
            app_ms1826_picture_adjust(u8Dev,i+VEHC_VIPSS_1,g_stMs1826Event[u8Dev].u8Hue[i],MS1826_ADJUST_HUE);
            LOG("TX[%d]        u8Hue: %d\r\n",i,g_stMs1826Event[u8Dev].u8Hue[i]);
        }
        if(g_stMs1826Event[u8Dev].bTxAudioSourceChangeFlag[i])
        {
            g_stMs1826Event[u8Dev].bTxAudioSourceChangeFlag[i] = FALSE;
//            if(g_bFirstStartupFlag[u8Dev][i] == FALSE)
            {
                app_ms1826_audio_input_switch(u8Dev,g_stMs1826Event[u8Dev].u8TxAudioSourceSta[i],i);
                LOG("TX[%d] audio source: %d\r\n",i,g_stMs1826Event[u8Dev].u8TxAudioSourceSta[i]);
            }
        }

        if(g_stMs1826Event[u8Dev].bTxHdmiAudioMuteChangeFlag[i])
        {
            g_stMs1826Event[u8Dev].bTxHdmiAudioMuteChangeFlag[i] = FALSE;
            if(g_stMs1826Event[u8Dev].u8TxHdmiAudioMuteSta[i]==0)
            {
    ...
    #endif
#endif
        if(g_stMs1826Event[u8Dev].bTxResolutionChangeFlag[i])
        {
            app_ms1826_output_resolution(u8Dev,i,g_stMs1826Event[u8Dev].u8TxResolutionID[i]);
            LOG("TX[%d] resolution ID: %d\r\n",i,g_stMs1826Event[u8Dev].u8TxResolutionID[i]);
            g_stMs1826Event[u8Dev].bTxResolutionChangeFlag[i] = FALSE;
        }
    }

    for(i=0;i<4;i++)
    {
        if(g_stMs1826Event[u8Dev].bRxEdidChangeFlag[i])
        {
            g_stMs1826Event[u8Dev].bRxEdidChangeFlag[i] = FALSE;
            app_ms1826_rx_edid_update(u8Dev,i,g_stMs1826Event[u8Dev].u8EdidBuf[i],256);
        }
        if(g_stMs1826Event[u8Dev].bOsdStringChangeFlag[i])
        {
            g_stMs1826Event[u8Dev].bOsdStringChangeFlag[i] = FALSE;
            LOG("TX[0x%02x] set string data\r\n",i);
            app_ms1826_display_user_string(u8Dev,i);
        }
        if(g_stMs1826Event[u8Dev].bVideoMuteChangeFlag[i])
        {
            g_stMs1826Event[u8Dev].bVideoMuteChangeFlag[i] = FALSE;
            _video_mute(u8Dev,i,g_stMs1826Event[u8Dev].bVideoMuteSta[i]);
        }
        if(g_stMs1826Event[u8Dev].bOsdStringDisplayChangeFlag[i])
        {
            g_stMs1826Event[u8Dev].bOsdStringDisplayChangeFlag[i] = FALSE;
            LOG("TX[0x%02x] set string %s\r\n",i,g_stMs1826Event[u8Dev].bOsdStringDisplayEnable[i]?"Enable":"Disable");
            app_ms1826_set_display_user_string_enable(u8Dev,i,g_stMs1826Event[u8Dev].bOsdStringDisplayEnable[i]);
        }
        if(g_stMs1826Event[u8Dev].bOsdStringColorChangeFlag[i])
        {
            g_stMs1826Event[u8Dev].bOsdStringColorChangeFlag[i] = FALSE;
            LOG("TX[0x%02x] set string color [%d,%d,%d]\r\n",i,
                    g_stMs1826Event[u8Dev].bOsdStringColorRed[i],
                    g_stMs1826Event[u8Dev].bOsdStringColorGreen[i],
                    g_stMs1826Event[u8Dev].bOsdStringColorBlue[i]);
            app_ms1826_set_user_string_color(u8Dev,i,
                    g_stMs1826Event[u8Dev].bOsdStringColorRed[i],
                    g_stMs1826Event[u8Dev].bOsdStringColorGreen[i],
                    g_stMs1826Event[u8Dev].bOsdStringColorBlue[i]);
        }
        if(g_stMs1826Event[u8Dev].bOsdStringPositionChangeFlag[i])
        {
            g_stMs1826Event[u8Dev].bOsdStringPositionChangeFlag[i] = FALSE;
            LOG("TX[0x%02x] set string position [%d,%d]\r\n",i,
                    g_stMs1826Event[u8Dev].u16OsdStringPositionX[i],
                    g_stMs1826Event[u8Dev].u16OsdStringPositionY[i]);
...
//            if(g_u8_display_mode[u8Dev] == MS1826_SEAMLESS_MATRIX_SWITCHER_OUT)
//                _video_freeze_out(u8Dev,sl_u8FreezeFlag);
//            else if(g_u8_display_mode[u8Dev] == MS1826_SEAMLESS_MATRIX_SWITCHER)
//                _video_freeze(u8Dev,sl_u8FreezeFlag);
            LOG("TX[0x%02x] sw freeze\r\n",sl_u8FreezeFlag);
        }

        if(g_stMs1826Event[u8Dev].bFreezeTimeEnd)
        {
            g_stMs1826Event[u8Dev].bFreezeSwitchFlag[0] = FALSE;
            g_stMs1826Event[u8Dev].bFreezeSwitchFlag[1] = FALSE;
            g_stMs1826Event[u8Dev].bFreezeSwitchFlag[2] = FALSE;
            g_stMs1826Event[u8Dev].bFreezeSwitchFlag[3] = FALSE;
    ...
    if(g_stMs1826Event[u8Dev].bWorkModeSwitch)
    {
        g_stMs1826Event[u8Dev].bWorkModeSwitch = FALSE;
        app_display_mode_cfg(u8Dev,g_stMs1826Event[u8Dev].u8WorkMode,g_stMs1826Event[u8Dev].u8SubMode);
    }

    if(g_stMs1826Event[u8Dev].bSplitterModeChangeFlag)
    {
        g_stMs1826Event[u8Dev].bSplitterModeChangeFlag = FALSE;
        app_ms1826_set_splitter_mode(u8Dev,g_stMs1826Event[u8Dev].u8SplitterMode);
    }

    if(g_stMs1826Event[u8Dev].bSplitUpdateChangeFlag)
    {
        g_stMs1826Event[u8Dev].bSplitUpdateChangeFlag = FALSE;
        app_ms1826_tx_split_update(u8Dev,g_stMs1826Event[u8Dev].u8SplitPath);
    }
}
    ...
    if(g_bPreMS1826SStbFlag[u8Dev] != sg_bMS1826StandbyFlag[u8Dev])
    {
        ...
        if(sg_bMS1826StandbyFlag[u8Dev])
        {
            for(i=0;i<4;i++)
            {
                ...
                if(g_bDigitalAudioSwByHdmiTx[u8Dev][i])
                    g_stMs1826Event[u8Dev].u8TxDigitalAudioSource[i] = u8_Tx_audio_sel[u8Dev][i];
                app_ms1826_audio_output_mute(u8Dev,g_stMs1826Event[u8Dev].u8TxDigitalAudioSource[i],i,TRUE);
            }
        }
        else
        {
