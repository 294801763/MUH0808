...
VOID app_osd_clear(UINT8 u8Dev)
{
	...
	for(u8_idx = 0; u8_idx < 4; u8_idx ++)
	{
		st_str.pstr = "               ";
		st_str.u16_len = 15;
		api_osd_show_length_string(&g_st_osd_dev[u8Dev][u8_idx], &st_str);
		st_str.u8_y = 1;
		api_osd_show_length_string(&g_st_osd_dev[u8Dev][u8_idx], &st_str);
		st_str.u8_y = 2;
		api_osd_show_length_string(&g_st_osd_dev[u8Dev][u8_idx], &st_str);
		...
		{
			g_st_misc_osd_mux[u8Dev][u8_idx].b_enable = MS_FALSE;
			g_st_misc_osd_mux[u8Dev][u8_idx].u8_osd_module = g_u8_misc_osd_idx[u8_mux];
			g_st_misc_osd_mux[u8Dev][u8_idx].u8_tx_module = g_u8_misc_module_tx[u8_idx];
...
#endif
		api_osd_colorspace_set(&g_st_osd_dev[u8Dev][u8_i], OSD_COLORSPACE_YCBCR444);
...
//	UINT8 u8_mux = 0;
    ...
    for(u8_i=0;u8_i<4;u8_i++)
    {
        ...
        {
            api_osd_clear(&g_st_osd_dev[u8Dev][u8_i]);
            api_osd_window_enable(&g_st_osd_dev[u8Dev][u8_i], OSD_WIN_DISPLAY, MS_FALSE);
            ...
            for(u8_mux = 0; u8_mux < 4; u8_mux ++)
            {
                g_st_misc_osd_mux[u8Dev][u8_i].b_enable = MS_FALSE;
...
VOID sys_show_osd(UINT8 u8Dev,BOOL b_show, UINT8 u8_index)
{
	...
	else
    {
		...
		api_osd_window_enable(&g_st_osd_dev[u8Dev][0], OSD_WIN_DISPLAY, MS_FALSE);
		api_osd_window_enable(&g_st_osd_dev[u8Dev][0], OSD_WIN_HIGHLIGHT1, MS_FALSE);
		api_osd_window_enable(&g_st_osd_dev[u8Dev][0], OSD_WIN_HIGHLIGHT2, MS_FALSE);
		api_osd_trigger(&g_st_osd_dev[u8Dev][0]);

		api_osd_window_enable(&g_st_osd_dev[u8Dev][1], OSD_WIN_DISPLAY, MS_FALSE);
		api_osd_trigger(&g_st_osd_dev[u8Dev][1]);

		api_osd_window_enable(&g_st_osd_dev[u8Dev][2], OSD_WIN_DISPLAY, MS_FALSE);
		api_osd_trigger(&g_st_osd_dev[u8Dev][2]);

		api_osd_window_enable(&g_st_osd_dev[u8Dev][3], OSD_WIN_DISPLAY, MS_FALSE);
