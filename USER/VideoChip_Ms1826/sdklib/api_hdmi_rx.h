...
{
    UINT8 u8_rx_idx;                                       /**< hdmi rx index @ref HDMI_RX_INDEX_E*/
...
* @param [in]   pst_rx_dev : RX模块对象指针
* @param [in]   u8_rx_idx  : hdmi rx index
* @note         该接口仅对RX模块指针对象实例初始化，如结构体清零及赋初始默认值，并没有对模块做初始化动作。
* @return       ERR_INV_PARM if input pointer is null\n
*               MS_ERR_OK if init successed
*/
ERR_TYPE_E api_hdmi_rx_instance_init(HDMI_RX_DEV_T *pst_rx_dev, UINT8 u8_rx_idx);
...
* @param [in]   pst_rx_dev : RX模块对象指针,releated para:u8_rx_idx
* @return
*/
...
* @param [in]   pst_rx_dev : releated para 1:u8_rx_idx 2:st_rx_ints(from b_freq_unlock to b_i2cmp_apblost)
* @return
*/
...
* @param [in]   pst_rx_dev : releated para 1:u8_rx_idx 2:st_rx_ints(from b_hs_act to b_rx_timing_unstb)
* @return
*/
...
* @param [in]   pst_rx_dev : releated para 1:u8_rx_idx 2:st_rx_ints(from b_pd_fifo_th_min_pass to b_drm_cks_chg)
* @return
*/
